...
import moderngl
ROOT=Path(__file__).resolve().parent;SNAP=ROOT/'source-911870882ae92597';RUN=ROOT/'fresh-gl-fixed-diagnostic';RUN.mkdir(exist_ok=True);holder={};events=[];servers=[]
def check(label):
 c=holder.get('ctx')
 if c is not None:
  error=c.error
...
spec=importlib.util.spec_from_file_location('pinned',SNAP/'export-wrapper.py');ex=importlib.util.module_from_spec(spec);spec.loader.exec_module(ex)
originalTree=ex.optimized_tree
def fixedTree(source):
 old=' target.release();targetTex.release();filterfbo.release();filtered.release();filtervao.release();filterprog.release();cube.release()'
 assert source.count(old)==1
 return originalTree(source.replace(old,' fbo.use()\n'+old))
ex.optimized_tree=fixedTree
manifest=json.loads((SNAP/'visual-snapshot.json').read_text());r=None
try:
 ...
 firstDepth=[tx.read() for tx,_ in r.shadows];key=r._staticShadowKey;r.render_frame_shadows();check('second cached shadow pass finished');secondDepth=[tx.read() for tx,_ in r.shadows];assert key is r._staticShadowKey and firstDepth==secondDepth;print('CACHED_SHADOW_REUSE_NO_ERROR',flush=True)
 (RUN/'passed.json').write_text(json.dumps({'freshEnvironment':True,'initializationNoError':True,'firstStaticShadowBuildNoError':True,'secondShadowCacheReuseNoError':True,'repeatedShadowDepthBitIdentical':True,'nativeCameraFramesRendered':0,'sourceFingerprint':manifest['fingerprint']},indent=2))
finally:
 for p in servers:
