# Daybreak — original solo piano master
...
The WAV measures **−19.00 LUFS**, **−1.65 dBTP**, with **7.7 LU loudness range**. The MP3 measures **−19.00 LUFS**, **−1.66 dBTP**, with the same measured loudness range. Both complete masters and all excerpts passed full decoding; every PCM value was finite, and no clipped samples were found.

The mix received a fixed −4.75 dB mastering gain. No compressor was used. A latency-compensated peak limiter catches only approximately 0.22 dB at the loudest possible peak. The full pre-master to master alignment was checked at three passages with zero detected sample lag. The performance's large-scale dynamics and attack crest were retained.
