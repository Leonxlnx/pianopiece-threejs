...
import { clamp, smooth, v3, lowerBound } from './math.mjs';
...
export class Pianist {
    ...
    async load(score, preparedModel) {
        ...
        this.model.position.set(0, -.344, .65);
    ...
    posture(time, energy, pedal) {
        ...
        for (const mesh of this.blinkMeshes) {
            ...
            set('eyeLookDownLeft', .15 + .07 * release);
            set('eyeLookDownRight', .15 + .07 * release);
    ...
    fingerPoints(base, target, f, fi, palmQ) {
    ...
    handPose(hand, notes, initialX, black) {
        const cacheKey = hand.side + notes.map(n => `${n.midi}/${n.finger}/${(n.contactLift ?? .002).toFixed(4)}`).sort().join(',');
        ...
        const cost = (p) => {
            ...
            for (const n of notes) {
                ...
                const z = this.contactDepth(base.z, black, fi), target = v3(keyX(n.midi), keySurfaceY(n.midi, z) + (n.contactLift ?? .002), z);
                const length = f.lengths.reduce((a, b) => a + b, 0);
                const d = target.distanceTo(base);
                const shape = this.fingerPoints(base, target, f, fi, q);
                if (fi > 0)
    ...
    update(time, score, piano, pedal, energy) {
        ...
        for (const hand of this.hands) {
            ...
            const shoulder = wp(hand.upper);
            const pole = v3((hand.side === 'L' ? -1 : 1) * (.30 + Math.abs(x) * .15), .83 - (1 - Math.exp(-pressure * .7)) * .014, .58);
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                idle.y = Math.max(idle.y, KEY_TOP + (idle.z < .230 ? .027 : .011));
                const keyTarget = (note) => { const v = piano.contact(note.midi, this.contactDepth(base.z, isBlack(note.midi), fi)); v.y += note.contactLift ?? .002; return v; };
                if (n)
                    target = keyTarget(n);
                else {
                ...
                if (!n && fi > 0) {
                    const localPose = (wristPosition, wristQ, touch) => {
                        const origin = finger.bones[0].position.clone().applyQuaternion(wristQ).add(wristPosition), chain = this.fingerPoints(origin, touch, finger, fi, wristQ), p = [origin, chain.pip, chain.dip, chain.tip];
                    ...
                    const contactPose = (note, at) => { const anchor = this.plannedPose(hand, at), origin = finger.bones[0].position.clone().applyQuaternion(anchor.q).add(anchor.position), touch = piano.contact(note.midi, this.contactDepth(origin.z, isBlack(note.midi), fi)); touch.y += note.contactLift ?? .002; return localPose(anchor.position, anchor.q, touch); };
                    const previous = prev && prev.time + prev.duration <= time ? prev : undefined, next = nxt && nxt.time >= time ? nxt : undefined;
                    const end = previous ? previous.time + previous.duration : -10, start = next?.time ?? 1e6, gap = start - end;
