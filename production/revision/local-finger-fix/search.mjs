...
const region=score.notes.filter(n=>n.hand==='R'&&n.time<184.7&&n.time+n.duration>181.3),ids=['p00874','p00877','p00878','p00879'];
...
for(let a=1;a<=5;a++)for(let b=1;b<=3;b++)for(let c=b+1;c<=4;c++)for(let d=c+1;d<=5;d++){
 const assignment=[a,b,c,d],ns=region.map(n=>{const i=ids.indexOf(n.id);if(i<0)return {...n};const finger=assignment[i];return {...n,finger,contactLift:finger===n.finger?n.contactLift:lifts[finger-1][isBlack(n.midi)?1:0]};});
...
candidates.sort((a,b)=>a.peakMean-b.peakMean||a.changes.length-b.changes.length);const report={baseline,candidates:candidates.slice(0,20)};fs.writeFileSync(root+'/search-report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
