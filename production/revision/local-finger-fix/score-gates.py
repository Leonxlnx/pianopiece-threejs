...
before=json.loads((r/'baseline-score.json').read_text());after=json.loads((r/'candidate-score.json').read_text())
old={n['id']:n for n in before['notes']};new={n['id']:n for n in after['notes']};errors=[]
for nid,a in old.items():
 b=new[nid]
 for k in ['time','duration','midi','velocity','hand','role']:
  if a[k]!=b[k]:errors.append(f'{nid} changed {k}')
 if not .04<=b['duration']<=a['duration']+.000001:errors.append(f'{nid} invalid release')
for k in before:
 if k!='notes' and before[k]!=after[k]:errors.append(f'changed top-level {k}')
held=0
