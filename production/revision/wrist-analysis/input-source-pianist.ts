...
export class Pianist {
 ...
 m.castShadow=true;m.receiveShadow=true;m.frustumCulled=false;const mats=Array.isArray(m.material)?m.material:[m.material];for(const mat of mats){const p=mat as THREE.MeshStandardMaterial;if(p.map)p.map.anisotropy=4;if(p.normalMap)p.normalMap.anisotropy=4;p.envMapIntensity=.5;}
 ...
 const w=Math.exp(-Math.pow((age+.12)/.38,2))*n.velocity;
 reach+=keyX(n.midi)*w;weight+=w;
 if(n.role==='melody'||n.hand==='R'){melodyX+=keyX(n.midi)*w;melodyWeight+=w;}
 if(age>-.09&&age<.6)strike+=n.velocity*(n.role==='bass'?1.35:n.role==='melody'?.8:.28)*Math.exp(-Math.pow((age-.07)/.16,2));
 }
 reach=weight?reach/weight:0;melodyX=melodyWeight?melodyX/melodyWeight:reach;
 ...
 set('eyeLookDownLeft',.15+.07*release);set('eyeLookDownRight',.15+.07*release);
 ...
 fingerPoints(base:THREE.Vector3,target:THREE.Vector3,f:FingerRig,fi:number,palmQ?:THREE.Quaternion){
 ...
 const cacheKey=hand.side+notes.map(n=>`${n.midi}/${n.finger}/${(n.contactLift??.002).toFixed(4)}`).sort().join(',');const cached=this.poseCache.get(cacheKey);if(cached)return cached;
 ...
 const idle=base.clone().add(v3(fi===0?(hand.side==='L'?1:-1)*reach*.35:0,-reach*.19,-reach*(fi===0?.81:.90)));idle.y=Math.max(idle.y,KEY_TOP+(idle.z<.230?.027:.011));
 const keyTarget=(note:Note)=>{const v=piano.contact(note.midi,this.contactDepth(base.z,isBlack(note.midi),fi));v.y+=note.contactLift??.002;return v;};
 ...
 if(previous&&next&&gap<.50){const u=clamp((time-end)/Math.max(.001,gap)),s=smooth(u);target=keyTarget(previous).lerp(keyTarget(next),s);target.y+=Math.sin(u*Math.PI)*Math.min(.043,.014+gap*.10);}
 ...
 const shape=this.fingerPoints(base,target,finger,fi);
