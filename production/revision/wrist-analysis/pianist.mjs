...
export class Pianist {
    ...
    posture(time, energy, pedal) {
        ...
        for (const n of this.score?.notes ?? []) {
            ...
            if (age > -.09 && age < .6)
                strike += n.velocity * (n.role === 'bass' ? 1.35 : n.role === 'melody' ? .8 : .28) * Math.exp(-Math.pow((age - .07) / .16, 2));
        }
        reach = weight ? reach / weight : 0;
        melodyX = melodyWeight ? melodyX / melodyWeight : reach;
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                if (n)
                    ...
                    if (previous && next && gap < .50) {
                        ...
                        target.y += Math.sin(u * Math.PI) * Math.min(.043, .014 + gap * .10);
                    }
                    else {
                        target = idle.clone();
                        if (previous) {
