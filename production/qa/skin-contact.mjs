...
for(const n of score.notes){
 ...
 piano.update(time,score.notes,pedal);p.update(time,score,piano,pedal,energy);model.updateMatrixWorld(true);body.skeleton.update();piano.group.updateMatrixWorld(true);
 const {gap:min,count}=keyPadContact(body,ids[n.hand+n.finger],piano.keys.get(n.midi).mesh);
...
const dest=process.argv[2]??'production/pop-revision/skin-contact.json';fs.writeFileSync(dest,JSON.stringify({report,records},null,2));console.log(JSON.stringify(report,null,2));
if(report.missing.length||report.belowMinus3||report.above3)process.exitCode=1;
