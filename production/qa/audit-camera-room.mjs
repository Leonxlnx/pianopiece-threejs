...
stage.group.traverse(o=>{if(o.isMesh&&!o.isInstancedMesh&&/wall|pier|lintel|door|ceiling|beam|lining|panel|reveal/i.test(o.name))architecture.push(o);});
...
const report={samples,architecturalMeshes:architecture.length,minimumOriginClearanceM:minClearance,failures,passed:failures.length===0&&minClearance>0,scope:'Room-origin clearance and central sightline at seven points in each shot, four viewport ratios; hands/performer occlusion requires separate visual review.'};
fs.writeFileSync('production/revision/camera-room-audit.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));if(!report.passed)process.exitCode=1;
