...
for (const [index, time] of times.entries()) {
  ...
  if(process.env.DAYBREAK_EXPORT_HAIR_PAIRS)fs.writeFileSync(path.join(out, `pair-${index}-before.json`), JSON.stringify({ time, ...before }));
  if(process.env.DAYBREAK_EXPORT_HAIR_PAIRS)fs.writeFileSync(path.join(out, `pair-${index}-after.json`), JSON.stringify({ time, ...after }));
}
