...
type Entry={id:number;side:string;influences:Influence[];amount:number;roll:number;forearmIndex:number;handIndex:number;targetInfluences?:Influence[]};
type Roll={lower:number;hand:number;restLocal:THREE.Quaternion;q:THREE.Quaternion;lowerMeshQ:THREE.Quaternion;lowerMeshInverseQ:THREE.Quaternion;pivot:THREE.Vector3};
/**
 ...
 */
export function createWristVolumeCorrector(T:typeof THREE,mesh:THREE.SkinnedMesh,{strength=1,smoothWeights=0,rollForearm=false}={}){
 const geometry=mesh.geometry.clone();mesh.geometry=geometry;
 const pos=geometry.attributes.position as THREE.BufferAttribute,norm=geometry.attributes.normal as THREE.BufferAttribute|undefined;
 const basePos=pos.array.slice(),baseNorm=norm?.array.slice();
 const si=geometry.attributes.skinIndex as THREE.BufferAttribute,sw=geometry.attributes.skinWeight as THREE.BufferAttribute;
 const names=mesh.skeleton.bones.map(b=>b.name),entries:Entry[]=[],used=new Set<number>();
 const smooth=(v:number)=>{v=Math.max(0,Math.min(1,v));return v*v*(3-2*v);};
 for(let id=0;id<pos.count;id++){
  const influences:Influence[]=[];let side='',forearm=0,hand=0,digits=0,other=0;
  for(let j=0;j<4;j++){
 ...
 if(smoothWeights>0){
  const canonical=new Map<string,number>(),weld:number[]=[],neighbors=new Map<number,Set<number>>(),forearms=new Map<number,number>(),sides=new Map<number,string>();
 ...
 const rolls=new Map<number,Roll>(),virtual={rigid:true,matrix:new T.Matrix4(),real:new T.Quaternion(),dual:new T.Quaternion()},rollMatrix=new T.Matrix4(),rollQ=new T.Quaternion(),identityQ=new T.Quaternion();
 ...
 function originalMorphed(id:number,kind:'position'|'normal',base:ArrayLike<number>,out:THREE.Vector3,delta:THREE.Vector3){
