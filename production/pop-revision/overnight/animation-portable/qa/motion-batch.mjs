...
for(const win of windows){
 ...
 for(let i=0;i<=Math.ceil((finish-begin)*fps);i++){
  const time=Math.min(begin+i/fps,finish);r.pose(time);const snapshot=[];
...
fs.writeFileSync(output,JSON.stringify(report,null,2));console.log(JSON.stringify({...report,rows:undefined,failures:failures.map(r=>({...r,bones:undefined,quaternions:undefined}))},null,2));
