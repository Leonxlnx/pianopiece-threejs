...
const r=await runtime('paired-approach/candidate'),sample=surfaceSampler(r),index=r.score.notes.find(n=>n.id==='db00518'),middle=r.score.notes.find(n=>n.id==='db00522'),arrival=structuredClone(middle.approachWaypoint),times=JSON.parse(fs.readFileSync('paired-approach/quick-times.json')),controls=JSON.parse(fs.readFileSync('paired-approach/grid-controls.json'));
...
const files=['grid-paired.mjs','surface-sampler.mjs','runtime-immutable.mjs','paired-approach/grid-controls.json','paired-approach/quick-times.json','paired-approach/candidate/score.json','paired-approach/candidate/compiled/pianist.mjs','/dev/shm/daybreak-a4-882c1c2d8c18/original-pianist.glb'],hash=()=>Object.fromEntries(files.map(f=>[f,crypto.createHash('sha256').update(fs.readFileSync(f)).digest('hex')])),before=hash();
for(const [i,profile]of controls.entries()){
 middle.approachWaypoint=structuredClone(arrival);index.releaseWaypoint=profile;let keyRegressions=0,pairRegressions=0,targetSevere=0,maxTargetDepth=0,pairIncidences=0,maxTipSpeed=0,maxDorsal=-90,previous;
