...
export interface FingerIdlePoseControl { afterRelease: number; beforeNext: number; blend: number; liftDegrees: number; sweepDegrees: number; }
export interface Note { idlePose?: FingerIdlePoseControl; approachWaypoint?: FingerReleaseWaypoint; releaseWaypoint?: FingerReleaseWaypoint; approachPose?: JointPoseControl; releasePose?: JointPoseControl; id: string; time: number; duration: number; midi: number; velocity: number; contactLift?: number; contactZ?: number; thumbOpposition?: number; hand: Hand; finger: number; role?:string; }
