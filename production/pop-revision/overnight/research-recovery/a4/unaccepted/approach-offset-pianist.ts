...
import { createWristVolumeCorrector } from './wrist-volume';
...
export class Pianist {
 ...
 skinVolume?:ReturnType<typeof createWristVolumeCorrector>;
 ...
 this.score=score;
 this.model=preparedModel??(await new GLTFLoader().loadAsync('/assets/pianist.glb')).scene;this.group.add(this.model);this.model.rotation.y=Math.PI;this.model.position.set(0,-.344,SEAT_Z);
 ...
 if(m.name==='Human'&&(m as THREE.SkinnedMesh).isSkinnedMesh)this.skinVolume=createWristVolumeCorrector(THREE,m as THREE.SkinnedMesh,{rollForearm:true,smoothWeights:8});
 ...
 if(fi>0){const forward=shape.pip.clone().sub(base).normalize();cost+=Math.pow(Math.max(0,forward.z+.24),2)*.015+Math.pow(Math.max(0,Math.abs(forward.x)-.48),2)*.03;}
 }
 ...
 const settings=(this as any).jointTransport??{};
 ...
 const clearanceWeight=smooth((time-ownEnd-((prev as any)?.releaseTravel?.duration??.16))/.080)*smooth((ownStart-time-((nxt as any)?.approachTravel?.duration??.20))/.080);
 ...
 if(route?.enabled&&gap>=.50&&route.duration>0&&route.duration<=gap-approach&&time-end<route.duration){
  ...
  const touch=contact.clone().lerp(restTouch,cruise);touch.x+=route.x*envelope;touch.z+=route.z*envelope;touch.y=mix(mix(contact.y,route.height,lift),restTouch.y,land);
  const rotations=localPose(wp(hand.wrist),pose.q,touch,0,route.roll*envelope);
  finger.bones.forEach((bone,j)=>{bone.quaternion.copy(rotations[j]);bone.updateWorldMatrix(false,true);});
  continue;
 }
 // A reviewed long approach moves above the key row before its final vertical landing.
 ...
 if(arrival?.enabled&&gap>=.50&&arrival.duration>0&&arrival.duration<=.20&&arrival.duration<=gap-release&&start-time<arrival.duration){
  ...
  const lift=smooth(u/liftEnd),land=smooth((u-landStart)/(1-landStart)),cruise=smooth((u-liftEnd)/(landStart-liftEnd)),envelope=lift*(1-land);
  const offsetReturn=lift*(1-smooth((u-.50)/.35));
  const touch=restTouch.clone().lerp(contact,cruise);touch.x+=arrival.x*offsetReturn;touch.z+=arrival.z*envelope;touch.y=mix(mix(restTouch.y,arrival.height,lift),contact.y,land);
 ...
 let liftWeight=0,liftDegrees=settings.arc??20,sweepDegrees=0;
 if(previous&&time-end<release){const u=smooth((time-end)/Math.max(.001,release));liftWeight=4*u*(1-u);liftDegrees=previous.releasePose?.liftDegrees??liftDegrees;sweepDegrees=previous.releasePose?.sweepDegrees??0;}
 ...
 if(liftWeight>0){
  ...
  if(sweepDegrees!==0)rotations[0].premultiply(new THREE.Quaternion().setFromAxisAngle(v3(0,0,-1),sweepDegrees*Math.PI/180*liftWeight));
 }
 finger.bones.forEach((bone,j)=>{bone.quaternion.copy(rotations[j]);bone.updateWorldMatrix(false,true);});
 continue;
 }
 ...
 supportedElbow(hand:HandRig,shoulder:THREE.Vector3,wrist:THREE.Vector3,palmQ:THREE.Quaternion,pressure:number){
  const side=hand.side==='L'?-1:1,a=hand.lower.position.length(),b=hand.wrist.position.length();
  const axis=wrist.clone().sub(shoulder),distance=clamp(axis.length(),.0001,a+b-.00001);axis.normalize();
