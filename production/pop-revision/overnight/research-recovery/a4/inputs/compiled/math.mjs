...
export const rand = (i) => { const x = Math.sin(i * 127.1 + 311.7) * 43758.5453123; return x - Math.floor(x); };
export function scoreEnergy(time, sections) {
    let index = sections.findIndex(s => time >= s.start && time < s.end);
    if (index < 0)
...
export function segment(parent, a, b, r, mat, r2 = r, segments = 12) {
    const m = new THREE.Mesh(new THREE.CylinderGeometry(r2, r, a.distanceTo(b), segments), mat);
...
export function placeSegment(m, a, b) { m.position.copy(a).add(b).multiplyScalar(.5); m.scale.y = a.distanceTo(b); m.quaternion.setFromUnitVectors(v3(0, 1, 0), b.clone().sub(a).normalize()); }
export function lowerBound(arr, time, get) { let a = 0, b = arr.length; while (a < b) {
    const m = (a + b) >>> 1;
    if (get(arr[m]) < time)
        a = m + 1;
    else
...
export function pedalPosition(time, events) {
