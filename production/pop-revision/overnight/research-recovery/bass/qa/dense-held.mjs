...
const taskModelFile=process.env.DAYBREAK_MODEL_PATH;
const here=path.dirname(new URL(import.meta.url).pathname);
const files=[`${dir}/score.json`,`${base}/score.json`,`${dir}/windows.json`,here+'/dense-held.mjs',here+'/key-pad-contact.mjs',taskModelFile,...[dir,base].flatMap(v=>fs.readdirSync(v+'/compiled').filter(f=>f.endsWith('.mjs')).map(f=>v+'/compiled/'+f))];
...
async function make(variant){
 ...
 const bytes=fs.readFileSync(taskModelFile),gltf=await loader.parseAsync(bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength),'');
...
const report={passed:bad.length===0&&maxDifferenceMm<1e-9,samples,times:ordered.length,minGapMm,maxGapMm,maxDifferenceMm,bad,scope:'Actual assigned-key distal pads at 240Hz throughout selected note holds and every simultaneous held note during their guarded release intervals; held boundaries offset only one microsecond to remain held.',inputHashes:before,inputsUnchanged:JSON.stringify(before)===JSON.stringify(hashes())};report.passed&&=report.inputsUnchanged;
