...
const consumedFiles=[...Object.values(files),path.resolve(a.times),fileURLToPath(import.meta.url),...fs.readdirSync(path.dirname(files.rig)).filter(f=>f.endsWith('.mjs')).map(f=>path.join(path.dirname(files.rig),f))];
const hashInputs=()=>Object.fromEntries([...new Set(consumedFiles)].sort().map(f=>[f,crypto.createHash('sha256').update(fs.readFileSync(f)).digest('hex')]));
const initialInputs=hashInputs();
...
const requestedTimes=sampleTimes();if(!requestedTimes.length||!requestedTimes.every(Number.isFinite))throw Error('Invalid inventory sample times');
const times=[...new Set(requestedTimes.map(t=>Math.max(0,Math.min(t,score.duration))))].sort((x,y)=>x-y);
...
for(const [sample,t]of times.entries()){
 ...
 if(sample%240===0)console.log(JSON.stringify({sample,total:times.length,flaggedRows:knownPositiveRows}));
}
