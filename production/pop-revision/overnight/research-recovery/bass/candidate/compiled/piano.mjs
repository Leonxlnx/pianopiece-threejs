...
function woodTexture(pending, name = 'spruce') {
...
export class GrandPiano {
    ...
    constructor() {
        ...
        const castWeb = (a, b, startWidth, endWidth, name) => {
            ...
            shape.closePath();
            const web = slab(shape, .018, gold, .824, this.group);
            web.name = name;
        };
        for (const [i, a, b] of [
            [0, [-.654, .32], [.626, .36]],
            ...
            castWeb(new THREE.Vector2(...a), new THREE.Vector2(...b), .061, .043, `Tapered cast plate web ${i + 1}`);
        ...
        for (const bass of [true, false]) {
            ...
            const vertices = [], indices = [], segments = 100, baseY = .820, half = bass ? .027 : .022;
            for (let i = 0; i <= segments; i++) {
                ...
                for (const [x, y] of profile)
                    vertices.push(p.x + side.x * x, y, p.z + side.z * x);
                if (i)
                    for (let j = 0; j < 8; j++) {
                        const a = (i - 1) * 8 + j, b = (i - 1) * 8 + (j + 1) % 8, c = i * 8 + j, d = i * 8 + (j + 1) % 8;
            ...
            const bridge = new THREE.Mesh(geometry, wood);
            bridge.name = bass ? 'Raised laminated bass bridge' : 'Laminated tenor and treble bridge';
        ...
        for (let i = 0; i < pinPositions.length; i++) {
            dummy.position.copy(pinPositions[i]);
            dummy.updateMatrix();
        ...
        const lidInside = slab(outline(.034), .009, lidVeneer, -.006, lidPivot);
        lidInside.position.x = .75;
        lidInside.name = 'Satin walnut lid underside';
        ...
        for (let i = 0; i < lidPositions.count; i++) {
            grainUV[i * 2] = lidPositions.getX(i) * .65;
            grainUV[i * 2 + 1] = lidPositions.getZ(i) * .75;
        }
        ...
        for (let i = 0; i < 3; i++)
            for (let j = 0; j < 2; j++) {
                ...
                button.scale.y = .22;
                button.position.set((i - 1) * .158, .498, .585 + j * .15);
                bench.add(button);
            }
        box(bench, .588, .035, .318, 0, .413, .66, lacquer, .006);
        for (const x of [-.251, .251])
            for (const z of [.532, .788]) {
                segment(bench, v3(x, .045, z), v3(x, .419, z), .019, lacquer, .023, 7);
                box(bench, .043, .027, .043, x, .019, z, brass, .004);
            }
