...
function block(parent, name, size, position, mat, r = .008) {
    ...
    if (mat.name === 'Honed limestone') {
        ...
        for (let i = 0; i < p.count; i++) {
            ...
            if (Math.abs(n.getX(i)) > .5)
                uv.setXY(i, z, y);
            else if (Math.abs(n.getY(i)) > .5)
                uv.setXY(i, x, z);
            else
                uv.setXY(i, x, y);
        }
...
export class Stage {
    ...
    constructor(scene, mobile) {
        ...
        for (let variant = 0; variant < 4; variant++) {
            ...
            const grain = map('/assets/materials/walnut.png', .28, 1.6);
            grain.offset.set(variant * .219, variant * .137);
        ...
        this.reflector.position.set(0, .001, -.55);
        ...
        for (const side of [-1, 1])
            for (let panel = 0; panel < 6; panel++) {
                ...
                for (const dz of [-.63, .63])
                    block(this.group, 'Panel walnut stile', [.071, 2.19, .025], [side * 7.106, 2.63, z + dz], wallWood, .003);
            }
        for (const z of [-3.8, .3]) {
            const seat = new THREE.Group();
            seat.name = 'Window-side lounge seat';
            seat.position.set(5.72, 0, z);
            seat.rotation.y = Math.PI / 2;
            this.group.add(seat);
            block(seat, 'Chair upholstered seat', [.76, .14, .70], [0, .48, 0], wool, .065);
            const back = block(seat, 'Chair upholstered back', [.76, .70, .13], [0, .82, .30], wool, .055);
            back.rotation.x = -.10;
            for (const x of [-.38, .38]) {
                block(seat, 'Chair walnut arm', [.055, .055, .77], [x, .70, .0], wood, .022);
                for (const zz of [-.27, .29])
                    block(seat, 'Chair solid leg', [.043, .68, .043], [x, .34, zz], wood, .007);
            }
        }
        ...
        for (const x of [5.33, 5.95])
            for (const z of [-2.07, -1.45])
                block(this.group, 'Side table bronze leg', [.022, .44, .022], [x, .22, z], bronze, .003);
        const profile = [[0, 0], [.092, 0], [.117, .045], [.122, .17], [.081, .265], [.054, .29], [.054, .315], [.044, .318], [.043, .294]].map(([x, y]) => new THREE.Vector2(x, y));
        const vase = new THREE.Mesh(new THREE.LatheGeometry(profile, 40), ceramic);
        vase.name = 'Hand-thrown stoneware';
        vase.position.set(5.62, .50, -1.73);
        vase.castShadow = true;
        vase.receiveShadow = true;
        this.group.add(vase);
        const lamp = new THREE.Group();
        lamp.name = 'Reading light';
        lamp.position.set(6.12, 0, -4.57);
        this.group.add(lamp);
        block(lamp, 'Lamp base', [.29, .025, .29], [0, .028, 0], bronze, .04);
        block(lamp, 'Lamp stem', [.016, 1.62, .016], [0, .84, 0], bronze, .006);
        const shade = new THREE.Mesh(new THREE.CylinderGeometry(.16, .235, .30, 40, 1, true), linen);
        shade.name = 'Linen lamp shade';
        shade.material = linen.clone();
        shade.material.side = THREE.DoubleSide;
        shade.position.y = 1.56;
        shade.castShadow = true;
        lamp.add(shade);
        const lampInner = new THREE.Mesh(new THREE.CircleGeometry(.205, 40), glow);
        lampInner.rotation.x = Math.PI / 2;
        lampInner.position.y = 1.42;
        lamp.add(lampInner);
...
#include <colorspace_fragment>
        ...
        const key = new THREE.SpotLight(0xffead4, 94, 25, .75, .86, 2);
        ...
        const fill = new THREE.SpotLight(0xc6dcf1, 92, 24, .85, .95, 2);
        fill.position.set(4.3, 3.5, 1.3);
        fill.target.position.set(0, .85, .2);
        scene.add(fill, fill.target);
        this.lights.push(fill);
        const sun = new THREE.SpotLight(0xffdbad, 265, 40, .64, .73, 2);
    ...
    batchArchitecture() {
        ...
        this.group.traverse(o => {
            const m = o;
            if (!m.isMesh || m.isInstancedMesh || Array.isArray(m.material))
                return;
            const mat = m.material;
            if (mat.transparent || mat.isShaderMaterial)
                return;
        ...
        for (const meshes of groups.values()) {
            if (meshes.length < 2)
                continue;
            const geometries = meshes.map(m => { const g = m.geometry.clone(); g.applyMatrix4(m.matrixWorld); g.clearGroups(); if (!g.index)
                return g; const flat = g.toNonIndexed(); g.dispose(); return flat; });
            const merged = mergeGeometries(geometries, false);
            geometries.forEach(g => g.dispose());
            if (!merged)
                continue;
            const first = meshes[0], batch = new THREE.Mesh(merged, first.material);
            batch.name = 'Pavilion · ' + first.material.name;
            batch.castShadow = first.castShadow;
            batch.receiveShadow = first.receiveShadow;
            batch.renderOrder = first.renderOrder;
            batch.layers.mask = first.layers.mask;
            batch.userData.architecture = first.userData.architecture;
            let vertexStart = 0;
        ...
        this.group.traverse(o => { const m = o; if (m.geometry)
            removed.delete(m.geometry); });
    ...
    update(time, energy, pulse) {
        ...
        this.lights[0].intensity = mix(88, 100, lift);
        this.lights[2].intensity = mix(240, 320, lift);
