...
SCORE_PATH=pathlib.Path(os.environ.get('DAYBREAK_SCORE_PATH', str(HERE/'score.json')))
SAMPLE_MANIFEST=pathlib.Path(os.environ.get('DAYBREAK_SAMPLE_MANIFEST_PATH', str(HERE/'sample-manifest.json')))
SCORE_BYTES=SCORE_PATH.read_bytes()
EXPECTED_SCORE_SHA=hashlib.sha256(SCORE_BYTES).hexdigest()
SR=44100
ROLE_GAIN={'melody':1.0,'harmony':.87,'bass':.85}
...
def render():
    ...
    # Freeze exactly the bytes parsed above, even if an external caller edits
    # its source while rendering. A changed source is rejected before success.
    (HERE/'score-frozen.json').write_bytes(SCORE_BYTES)
...
def mix(buses):
    ...
    counter_stats={role:db(rms(bus[counter_mask])) for role,bus in buses.items()} if counter_mask.any() else {}
    manifest={
...
# Independent MP3 frames avoid a reproducible decoder reservoir error in the
# final-refrain excerpt on the installed FFmpeg/LAME combination.
def encode_master(linear_gain,limiter=False):
    ...
    command(['ffmpeg','-nostdin','-y','-v','error','-i',str(HERE/'daybreak-solo-master.wav'),
        '-c:a','libmp3lame','-b:a','320k','-reservoir','0','-write_xing','1','-id3v2_version','3',
...
def codec_compare(reference,decoded):
    checks=[]
    for at in [1.0,35.0,75.0,150.0,190.0,219.0]:
...
def excerpts():
    starts=[('first-theme',70.0,30.0),('climax',180.0,30.0)]
    result=[]
    for label,start,duration in starts:
        ...
        command(['ffmpeg','-nostdin','-y','-v','error','-i',str(path),'-c:a','libmp3lame','-b:a','320k','-reservoir','0',str(mp3)])
...
if __name__=='__main__':
    ...
    if not args.master_only:
        ...
        del buses
    master()
    assert sha(SCORE_PATH)==EXPECTED_SCORE_SHA,'Source changed during rendering; rerender from the new frozen score.'
    import platform, scipy
    analysis=json.loads((HERE/'mastering-analysis.json').read_text())
    write_json(HERE/'reproduction-manifest.json',{
        'scoreSha256':EXPECTED_SCORE_SHA,'sampleManifestSha256':sha(SAMPLE_MANIFEST),
        'rendererSha256':sha(__file__),'composerSha256':sha(HERE/'compose.py'),
