...
On the identical 22-note active-grip score, old idle produced 22 severe key intersections across 19 of 72 contexts. The replacement has zero severe key rows in that screen and in 167 additional dense transition samples. Opposing-hand rows fall from three to zero. The original 60 Hz replay missed higher speed peaks; final combined movement verification now uses 240 Hz and the stricter 1.5 m/s wrist, 25 m/s² wrist acceleration and 5 m/s fingertip limits. It also screens actual dorsal-palm finger elevation at 60° and backward PIP movement at 3 mm, conservative project review bounds rather than clinical human limits. The shared skin-normal helper exactly matches 543 independent closeup measurements; this validates the measurement, not the unfinished full animation.
...
Music scripts use their own directory for outputs. Fetch the pinned sample bank with `fetch_samples.py`, then run `render.py` and `verify.py --score ../../../public/assets/score.json` from `production/pop-revision/music/`. Read its composition and provenance files first. WAV intermediates and the sample bank are reproducible and intentionally ignored. Finger-only changes preserve the audible-event signature. For final delivery, render from the exact integrated score using `DAYBREAK_SCORE_PATH=../../../public/assets/score.json python render.py --no-stems`, then run the verifier and copy the resulting MP3 to `public/assets/daybreak.mp3`. The optional `--no-stems` skips only dry role cache files to save about 240 MB; fresh sample rendering, the premaster, mastering, source hashes and codec checks remain unchanged. A final immutable film job must bind its final score to matching verified audio; never relabel an older reproduction manifest.
