...
#include <colorspace_fragment>
 ...
 const list=groups.get(mat)??[];list.push(m);groups.set(mat,list);
 });
 for(const [material,meshes] of groups){
 if(meshes.length<2)continue;
 const geometries=meshes.map(m=>{const g=m.geometry.clone();g.applyMatrix4(m.matrixWorld);g.clearGroups();return g.index?g.toNonIndexed():g;});
 const merged=mergeGeometries(geometries,false);geometries.forEach(g=>g.dispose());if(!merged)continue;
 const batch=new THREE.Mesh(merged,material);batch.name='Architecture · '+material.name;batch.castShadow=meshes.some(m=>m.castShadow);batch.receiveShadow=true;
 for(const m of meshes){m.removeFromParent();m.geometry.dispose();}this.group.add(batch);
 }
 }
