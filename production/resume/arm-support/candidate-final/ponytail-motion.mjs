...
import { HAIR_RATE, HAIR_QUANTUM, HAIR_DURATION, HAIR_SAMPLES } from './hair-motion-data.mjs';
...
export function ponytailMotion(bones, rest) {
    ...
    return (time) => {
        ...
        for (let i = 0; i < chain.length; i++) {
            const { bone, rest: neutral } = chain[i];
            if (!bone || !neutral)
                continue;
            const k = i * 2;
            ...
            delta.setFromEuler(euler.set(x, 0, z));
            bone.quaternion.copy(neutral).multiply(delta);
        }
    };
}
