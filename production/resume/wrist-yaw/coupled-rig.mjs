...
export class Pianist {
    ...
    plannedPose(hand, time) {
        if (this.wristMotion) { const pose=this.wristMotion(hand.side,time); if(hand.side==='L'&&globalThis.daybreakYawPose){const p=globalThis.daybreakYawPose;pose.position.add(new THREE.Vector3(p[0],p[1],p[2]));pose.q.premultiply(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0,1,0),p[3]));} return pose; }
