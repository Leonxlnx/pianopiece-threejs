...
import { clamp, smooth, mix, v3, lowerBound } from '/workspace/scratch/2e8cc8e77f98/curled-idle/compiled/math.mjs';
import { keyX, isBlack, KEY_TOP, keySurfaceY } from '/workspace/scratch/2e8cc8e77f98/curled-idle/compiled/piano.mjs';
import { wristMotionSampler } from '/workspace/scratch/2e8cc8e77f98/curled-idle/compiled/wrist-motion.mjs';
import { ponytailMotion } from '/workspace/scratch/2e8cc8e77f98/curled-idle/compiled/ponytail-motion.mjs';
        ...
        for (const hand of this.hands) {
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                if (!n && fi > 0) {
                    ...
                    const idlePose = finger.rest.map(q => q.clone());
                    const bends = [60, -55, -25], parentLocal = new THREE.Quaternion();
                    for (let j = 0; j < 3; j++) {
                        const axis = v3(-1, 0, 0).applyQuaternion(parentLocal.clone().invert());
                        idlePose[j].premultiply(new THREE.Quaternion().setFromAxisAngle(axis, bends[j] * Math.PI / 180));
                        parentLocal.multiply(idlePose[j]);
                    }
                    const contactPose = (note, at) => { const anchor = this.plannedPose(hand, at), origin = finger.bones[0].position.clone().applyQuaternion(anchor.q).add(anchor.position), z = note.contactZ ?? this.contactDepth(origin.z, isBlack(note.midi), fi), touch = v3(keyX(note.midi), keySurfaceY(note.midi, z, 1) + (note.contactLift ?? .002), z); return localPose(anchor.position, anchor.q, touch, note.thumbOpposition ?? 0); };
                    const previous = prev && prev.time + prev.duration <= time ? prev : undefined, next = nxt && nxt.time >= time ? nxt : undefined;
                    const end = previous ? previous.time + previous.duration : -10, start = next?.time ?? 1e6, gap = start - end;
                    ...
                    // Apply the complete local pose without returning to world-target IK.
                    finger.bones.forEach((bone, j) => { bone.quaternion.copy(rotations[j]); bone.updateWorldMatrix(false, true); });
                    continue;
                }
                for (let j = 0; j < 3; j++) {
