        ...
        for (const hand of this.hands) {
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                if (!n && fi === 0) {
                    ...
                    const contactPose = (note, at) => { const anchor = this.plannedPose(hand, at), origin = finger.bones[0].position.clone().applyQuaternion(anchor.q).add(anchor.position), z = note.contactZ ?? this.contactDepth(origin.z, isBlack(note.midi), fi), touch = v3(keyX(note.midi), (hand.side === 'R' && prev?.id === 'p00194' && nxt?.id === 'p00197' && note.id === 'p00197' && time >= 45.01123 && time <= 45.388676 ? piano.contact(note.midi, z).y : keySurfaceY(note.midi, z, 1)) + (note.contactLift ?? .002), z); return localPose(anchor.position, anchor.q, touch, note.thumbOpposition ?? 0); };
                    const previous = prev && prev.time + prev.duration <= time ? prev : undefined, next = nxt && nxt.time >= time ? nxt : undefined;
                    const end = previous ? previous.time + previous.duration : -10, start = next?.time ?? 1e6, gap = start - end;
                    ...
                    if (hand.side === 'R' && previous?.id === 'p00194' && next?.id === 'p00197' && time > 45.01123 && time < 45.10) {
                        const angle = -.20 * (time < 45.031 ? smooth((time - 45.01123) / (45.031 - 45.01123)) : time <= 45.071 ? 1 : 1 - smooth((time - 45.071) / (.029)));
                        rotations[0].premultiply(new THREE.Quaternion().setFromAxisAngle(v3(0, 0, 1), angle));
                    }
