        ...
        for (const hand of this.hands) {
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                if (!n && fi === 0) {
                    ...
                    if (hand.side === 'R' && previous?.id === 'p00852' && next?.id === 'p00855' && time > 179.943184 && time < 180.304012) {
                        let i = 1;
                        while (THUMB_REVIEW_CURVE[i].t < time)
                            i++;
                        const a = THUMB_REVIEW_CURVE[i - 1], b = THUMB_REVIEW_CURVE[i], dt = b.t - a.t, u = (time - a.t) / dt, offset = a.offset.map((v, k) => (2 * u ** 3 - 3 * u * u + 1) * v + (u ** 3 - 2 * u * u + u) * dt * a.velocity[k] + (-2 * u ** 3 + 3 * u * u) * b.offset[k] + (u ** 3 - u * u) * dt * b.velocity[k]);
                        rotations[0].premultiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(offset[0], offset[1], offset[2], 'XYZ')));
                    }
