...
export class Pianist {
 ...
 const idleOrigin=finger.bones[0].position.clone().applyQuaternion(pose.q).add(wp(hand.wrist)),idleTouch=idleOrigin.clone().add(v3((hand.side==='R'?1:-1)*.55,.70,.10).multiplyScalar(reach).applyQuaternion(pose.q));
 const idlePose=localPose(wp(hand.wrist),pose.q,idleTouch);
