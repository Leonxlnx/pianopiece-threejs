        ...
        for (const hand of this.hands) {
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                if (!n && fi === 0) {
                    ...
                    // Introduce the compact reference only in settled rests. Exact held
                    // anchors retain their original local roll throughout release/approach.
                    const idlePose = localPose(wp(hand.wrist), pose.q, idle), compact = finger.rest.map(q => q.clone());
                    compact[0].premultiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(-.85, hand.side === 'L' ? -.30 : .30, hand.side === 'L' ? .30 : -.30, 'XYZ')));
                    const since = prev && prev.time + prev.duration <= time ? time - prev.time - prev.duration : 100;
                    const until = nxt && nxt.time >= time ? nxt.time - time : 100;
                    const restBlend = smooth((Math.min(since, until) - .10) / .20);
                    idlePose.forEach((q, j) => q.slerp(compact[j], restBlend));
