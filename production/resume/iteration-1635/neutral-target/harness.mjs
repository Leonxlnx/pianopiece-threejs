...
const {Pianist}=await import(process.env.DAYBREAK_RIG_MODULE?pathToFileURL(process.env.DAYBREAK_RIG_MODULE).href:'./compiled/pianist.mjs');
...
const {wristMotionSampler}=await import(process.env.DAYBREAK_WRIST_MODULE?pathToFileURL(process.env.DAYBREAK_WRIST_MODULE).href:'./compiled/wrist-motion.mjs');
...
const bytes=fs.readFileSync('/workspace/scratch/2e8cc8e77f98/neutral-target/pianist.glb');
const gltf=await loader.parseAsync(bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength),'');
export const baseline=JSON.parse(fs.readFileSync('baseline-score.json'));
