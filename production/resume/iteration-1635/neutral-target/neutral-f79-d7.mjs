...
import { clamp, smooth, mix, v3, lowerBound } from '/workspace/scratch/2e8cc8e77f98/neutral-target/compiled/math.mjs';
import { keyX, isBlack, KEY_TOP, keySurfaceY } from '/workspace/scratch/2e8cc8e77f98/neutral-target/compiled/piano.mjs';
import { wristMotionSampler } from '/workspace/scratch/2e8cc8e77f98/neutral-target/compiled/wrist-motion.mjs';
import { ponytailMotion } from '/workspace/scratch/2e8cc8e77f98/neutral-target/compiled/ponytail-motion.mjs';
        ...
        for (const hand of this.hands) {
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                if (!n && fi > 0) {
                    ...
                    // Curved inactive chain moves with the palm; no world-key neutral target.
                    const neutral = base.clone().add(v3(0, reach * 0.79, reach * 0.07).applyQuaternion(pose.q));
                    const idlePose = localPose(wp(hand.wrist), pose.q, neutral);
                    const contactPose = (note, at) => { const anchor = this.plannedPose(hand, at), origin = finger.bones[0].position.clone().applyQuaternion(anchor.q).add(anchor.position), z = note.contactZ ?? this.contactDepth(origin.z, isBlack(note.midi), fi), touch = v3(keyX(note.midi), keySurfaceY(note.midi, z, 1) + (note.contactLift ?? .002), z); return localPose(anchor.position, anchor.q, touch, note.thumbOpposition ?? 0); };
                    const previous = prev && prev.time + prev.duration <= time ? prev : undefined, next = nxt && nxt.time >= time ? nxt : undefined;
                    const end = previous ? previous.time + previous.duration : -10, start = next?.time ?? 1e6, gap = start - end;
                    let rotations = idlePose.map(q => q.clone());
                    ...
                    // Apply complete local endpoint interpolation without world-target fallback.
                    finger.bones.forEach((bone, j) => { bone.quaternion.copy(rotations[j]); bone.updateWorldMatrix(false, true); });
                    continue;
                }
                for (let j = 0; j < 3; j++) {
