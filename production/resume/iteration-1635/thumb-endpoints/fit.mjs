...
const ids=['p00193','p00194','p00855','p00862','p01008'];
...
for(const id of ids){const n=s.notes.find(n=>n.id===id),original=structuredClone(n);let best=null,tried=0;
 ...
 const evalP=p=>{Object.assign(n,{contactZ:p[0],contactLift:p[1],thumbOpposition:p[2]});install(s);let key=0,own=0,neighbor=0,contact=0,gap=-Infinity;
  ...
  const loss=Math.max(0,key-2.3)*50+own*.7+neighbor*.7+Math.max(0,contact-.1)*150+Math.max(0,gap-2)*50+Math.abs(p[2])*.15;
 ...
 Object.assign(n,{contactZ:best.p[0],contactLift:best.p[1],thumbOpposition:best.p[2]});install(s);results.push({id,original,tried,best});console.log(JSON.stringify(results.at(-1)));fs.writeFileSync('candidate-score.json',JSON.stringify(s));fs.writeFileSync('fit-report.json',JSON.stringify(results,null,2));
}
