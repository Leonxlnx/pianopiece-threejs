...
s.notes.find(n=>n.id==='p00855').contactLift+=.0015;s.notes.find(n=>n.id==='p01008').contactLift+=.001;
...
fs.writeFileSync('three-lift-'+label+'-rows.json',JSON.stringify(rows));reports.push({label,rows:rows.length});}
fs.writeFileSync('three-lift-candidate.json',JSON.stringify(s));console.log(JSON.stringify(reports));
