...
for(const finger of [2,3,4,5]){let best;const evalP=p=>{for(const n of notes)Object.assign(n,{finger,contactZ:p[0],contactLift:p[1],thumbOpposition:0});install(s);let core=0,pairs=0,error=0,gap=0;
 for(const time of times){const r=measure(time,'R');core=Math.max(core,...r.keyHits.filter(h=>h.active||h.patch==='RPalm').map(h=>h.depth));pairs=Math.max(pairs,r.crossings.reduce((s,c)=>s+c.trianglePairs,0));error=Math.max(error,...r.contacts.map(c=>c.error*1000));gap=Math.max(gap,...r.meshContacts.map(c=>Math.max(c.minGap??100,-(c.minGap??0))));}
 ...
 for(const n of notes)Object.assign(n,{finger,contactZ:best.p[0],contactLift:best.p[1],thumbOpposition:0});install(s);results.push(best);fs.writeFileSync('refinger-'+finger+'.json',JSON.stringify(s));console.log(JSON.stringify(best));
}fs.writeFileSync('refinger-report.json',JSON.stringify(results,null,2));
