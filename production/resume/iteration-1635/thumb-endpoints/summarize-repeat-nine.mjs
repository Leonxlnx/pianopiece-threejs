...
const a=JSON.parse(fs.readFileSync('repeat-nine-baseline-rows.json')),b=JSON.parse(fs.readFileSync('repeat-nine-candidate-rows.json'));if(a.length!==b.length)throw Error('rows');const failures=[],counts={heldSamples:0,maxHeldCore:0,maxHeldAbsPadGap:0,maxHeldContactMm:0,newCore:0,newPairs:0,newPad:0};
...
fs.writeFileSync('repeat-nine-validation.json',JSON.stringify({counts,failures},null,2));console.log(JSON.stringify({counts,first:failures.slice(0,8)}));
