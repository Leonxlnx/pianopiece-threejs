...
for(const id of ids){const n=s.notes.find(n=>n.id===id),original=structuredClone(n);let best=null,tried=0;
 ...
 const evalP=p=>{Object.assign(n,{contactZ:p[0],contactLift:p[1],thumbOpposition:p[2]});install(s);let key=0,own=0,neighbor=0,contact=0,gap=-Infinity;
  ...
  if(!best||result.loss<best.loss){best=result;fs.writeFileSync(id+'-wide-best.json',JSON.stringify({id,original,tried,best},null,2));}
  return result;
 };
 const z=isBlack(n.midi)?[.174,.188,.202,.210]:[.225,.24,.255,.27];
 for(const depth of z)for(const lift of [.001,.003,.005,.007])for(const opposition of [-3,-2.4,-1.8,-1.2,-.6,0,.6,1.2,1.8,2.4,3])evalP([depth,lift,opposition]);
 for(const step of [.01,.005,.0025,.001])for(let pass=0;pass<2;pass++){const base=[...best.p];for(let axis=0;axis<3;axis++)for(const sign of [-1,1]){const p=[...base];p[axis]+=sign*(axis===0?step:axis===1?step*.25:step*12);if(p[0]<.165||p[0]>.279||p[1]<.0005||p[1]>.01||Math.abs(p[2])>3.14)continue;evalP(p);}}
 Object.assign(n,{contactZ:best.p[0],contactLift:best.p[1],thumbOpposition:best.p[2]});install(s);results.push({id,original,tried,best});console.log(JSON.stringify(results.at(-1)));fs.writeFileSync('candidate-wide-opposition.json',JSON.stringify(s));fs.writeFileSync('wide-opposition-report.json',JSON.stringify(results,null,2));
}
