...
const a=JSON.parse(fs.readFileSync('three-lift-baseline-rows.json')),b=JSON.parse(fs.readFileSync('three-lift-candidate-rows.json'));if(a.length!==b.length)throw Error('rows');const failures=[],counts={heldSamples:0,maxHeldCore:0,maxHeldAbsPadGap:0,maxHeldContactMm:0,newCore:0,newPairs:0,newPad:0};
const ids=['p00855','p00862','p01008'];
...
fs.writeFileSync('three-lift-validation.json',JSON.stringify({counts,failures},null,2));console.log(JSON.stringify({counts,first:failures.slice(0,8)}));
