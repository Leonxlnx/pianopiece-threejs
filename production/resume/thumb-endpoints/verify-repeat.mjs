...
const s=JSON.parse(fs.readFileSync('grip-repeat-e5-supported-candidate.json'));
...
fs.writeFileSync('repeat-e5-'+label+'-rows.json',JSON.stringify(rows));reports.push({label,rows:rows.length});}
fs.writeFileSync('repeat-e5-supported-candidate.json',JSON.stringify(s));console.log(JSON.stringify(reports));
