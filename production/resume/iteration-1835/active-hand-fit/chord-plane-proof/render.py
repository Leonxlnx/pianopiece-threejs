...
root=Path.cwd();leaf=root/'chord-plane-proof';rig=Path(sys.argv[1]).resolve();label=sys.argv[2];score=Path(sys.argv[3]).resolve();out=leaf/'renders'/label;out.mkdir(parents=True,exist_ok=True)
rows=json.load(open(leaf/'v6-full-candidate-rows.json'));chosen=[]
for t,side in [(198.9945185,'R'),(199.504266,'R'),(199.8365935,'R'),(200.030172655,'R')]:chosen.append(min([q for q in rows if q['side']==side],key=lambda q:abs(q['time']-t)))
results={};index=[]
