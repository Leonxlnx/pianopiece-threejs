...
export class Pianist {
 ...
 const controls=((globalThis as any).__thumbRestCandidates??[]).find((c:any)=>c.previous===previous?.id&&c.next===next?.id&&Math.abs(end-c.end)<1e-7&&Math.abs(start-c.start)<1e-7);
