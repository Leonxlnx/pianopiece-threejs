...
import { clamp, smooth, mix, v3, lowerBound } from '/workspace/scratch/2e8cc8e77f98/rh-thumb-rest-extension/compiled/math.mjs';
import { keyX, isBlack, KEY_TOP, keySurfaceY } from '/workspace/scratch/2e8cc8e77f98/rh-thumb-rest-extension/compiled/piano.mjs';
import { wristMotionSampler } from '/workspace/scratch/2e8cc8e77f98/rh-thumb-rest-extension/compiled/wrist-motion.mjs';
import { ponytailMotion } from '/workspace/scratch/2e8cc8e77f98/rh-thumb-rest-extension/compiled/ponytail-motion.mjs';
        ...
        for (const hand of this.hands) {
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                if (!n && fi === 0) {
                    ...
                    const controls = (globalThis.__thumbRestCandidates ?? []).find((c) => c.previous === previous?.id && c.next === next?.id && Math.abs(end - c.end) < 1e-7 && Math.abs(start - c.start) < 1e-7);
