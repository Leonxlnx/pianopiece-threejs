...
excluded = json.loads((OUT/'excluded-records.json').read_text()) if (OUT/'excluded-records.json').exists() else []
paths = [
...
for rel in paths:
    ...
    for kind in ['supportedIndexGaps','curves']:
        for index, c in enumerate(source.get(kind,[])):
            ...
            errors = []
            if any(e['source']==rel and e['kind']==kind and e['index']==index and e['curve']==c for e in excluded):
                errors.append('Rejected by combined whole-support geometry replay')
