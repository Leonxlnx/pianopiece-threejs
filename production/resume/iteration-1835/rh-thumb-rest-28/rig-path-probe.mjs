...
import { clamp, smooth, mix, v3, lowerBound } from '/workspace/scratch/2e8cc8e77f98/rh-thumb-rest-28/compiled/math.mjs';
import { keyX, isBlack, KEY_TOP, keySurfaceY } from '/workspace/scratch/2e8cc8e77f98/rh-thumb-rest-28/compiled/piano.mjs';
import { wristMotionSampler } from '/workspace/scratch/2e8cc8e77f98/rh-thumb-rest-28/compiled/wrist-motion.mjs';
import { ponytailMotion } from '/workspace/scratch/2e8cc8e77f98/rh-thumb-rest-28/compiled/ponytail-motion.mjs';
        ...
        for (const hand of this.hands) {
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                if (!n && fi === 0) {
                    ...
                    // Exact released-thumb gap diagnostic; complete authored-frame local poses.
                    const controls = globalThis.__thumbRestCandidate;
                    if (controls && hand.side === 'R' && previous?.id === 'p00112' && next?.id === 'p00142' && time > end && time < start) {
                        const w = smooth((time - end - controls.delay) / controls.fade) * smooth((start - time - controls.delay) / controls.fade);
                        ...
                        const a = contactPose(previous, end), b = contactPose(next, start), release = smooth((time - end) / (controls.release ?? .5)), arrival = smooth(1 - (start - time) / (controls.arrival ?? .5));
                        rotations = a.map((q, j) => q.slerp(relaxed[j], release).slerp(b[j], arrival));
                    }
