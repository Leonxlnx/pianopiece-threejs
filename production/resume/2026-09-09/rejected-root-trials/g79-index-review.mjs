...
const candidate=structuredClone(baseline);for(const id of ['p00181','p00659'])Object.assign(candidate.notes.find(n=>n.id===id),{finger:2,contactZ:.239,contactLift:.002});
...
const report={states:times.size,newCore,newOwn,newPair,newPad,beforeBad,afterBad,regressions};fs.writeFileSync('g79-index-review.json',JSON.stringify(report));fs.writeFileSync('g79-index-score.json',JSON.stringify(candidate));console.log(JSON.stringify({...report,regressions:regressions.slice(0,1)}));
