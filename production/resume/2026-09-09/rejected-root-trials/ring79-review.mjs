...
const candidate=structuredClone(baseline);for(const id of ['p00181','p00659'])candidate.notes.find(n=>n.id===id).contactLift+=.0008;
...
const report={states:times.size,newCore,newOwn,newPair,newPad,beforeBad,afterBad,regressions};fs.writeFileSync('ring79-review.json',JSON.stringify(report));fs.writeFileSync('ring79-score.json',JSON.stringify(candidate));console.log(JSON.stringify({...report,regressions:regressions.slice(0,1)}));
