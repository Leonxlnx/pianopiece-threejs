...
const candidate=structuredClone(baseline);Object.assign(candidate.notes.find(n=>n.id==='p00299'),{finger:5,contactZ:.252,contactLift:.002});
const spans=[[65.563664,68.448307]],times=new Set();for(const [begin,end]of spans){times.add(begin);times.add(end);for(let i=Math.ceil(begin*120);i<=Math.floor(end*120);i++)times.add(i/120);}
...
const report={states:times.size,newCore,newOwn,newPair,newPad,beforeBad,afterBad,regressions};fs.writeFileSync('p299-pinky-review.json',JSON.stringify(report));fs.writeFileSync('p299-pinky-score.json',JSON.stringify(candidate));console.log(JSON.stringify({...report,regressions:regressions.slice(0,1)}));
