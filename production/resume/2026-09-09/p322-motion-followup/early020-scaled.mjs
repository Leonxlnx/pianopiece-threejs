        ...
        for (const hand of this.hands) {
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                if (!n && fi > 0) {
                    ...
                    const bassOriginalContacts = { "p00322": { "time": 71.178428, "duration": 0.249968, "contactZ": null, "contactLift": 0.0062824555694730895 } };
                    ...
                    if (next)
                        weight = Math.min(weight, boundEnvelope(next, start, previous && next && gap < .50 ? smooth((time - end) / Math.max(.001, gap)) : smooth(1 - (start - time) / .27)));
