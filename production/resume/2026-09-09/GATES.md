# Gates: Daybreak continued revision, 2026-09-09

Scope: Recover the reviewed working source, resolve the remaining visible performance defects, publish the revised original performance and provide the complete desktop film.

- [x] G1: Recover the existing Sites checkpoint and reconstruct v11 with verified hashes and modular pose parity.
  EVIDENCE: app-parity.json: 13,356 time samples, 54 full Human mesh samples, exact zero local quaternion/world matrix/skin differences. Guarded restore and targeted strict performance typecheck passed.
- [ ] G2: Complete the interrupted idle replay and address the remaining visible raised-finger and joint-twist defects, using actual mesh and moving-key evidence.
  EVIDENCE: Full frozen-v11 idle replay completed; new cutoff/RH transfer/bass geometry deltas merged with exact parity at 1,101 states and 13 full Human skins. See merged-review/README.md and leaf-evidence. Remaining finite defects and motion tradeoffs prevent a whole-hand completion claim.
- [ ] G3: Master the original revised score with the licensed samples and verify exact score, audio duration and synchronization.
  EVIDENCE: pending
- [ ] G4: Build and verify the revised browser performance through the supported browser path.
  EVIDENCE: Strict performance TypeScript passes. Supported browser audio-only transport completed; WebGL reports disabled. Fresh production build and final-source playback remain pending.
- [ ] G5: Export, inspect, verify and save the full revised desktop film.
  EVIDENCE: pending
- [ ] G6: Publish the revision to the existing public Site and verify public delivery.
  EVIDENCE: pending
- [ ] G7: Push the complete current source, user prompts, evidence and continuation context to GitHub main.
  EVIDENCE: GitHub main advanced to 1aa9f90a87471bf8edccf59d211f95c88da303c3 with recovered app, latest prompts/context and selected reports. This precedes the newest merged leaf changes; complete historical resume archives remain in Sites main and await GitHub synchronization.
- [ ] G8: Critically listen to the new full song and verify full live playback.
  EVIDENCE: pending

Work resumed by explicit user request: "ok dann mach weiter bis Du fertig bist schätz etwa Zeit". Initial estimate 6–10 hours with 4–6 hours native rendering, conditional on recovery and defects. No claim of prior background work. Local scratch was empty; source recovery in progress. Prior outage interrupted, not completed, the old gates. Keep all earlier qualified evidence without pretending unqualified trials passed.
