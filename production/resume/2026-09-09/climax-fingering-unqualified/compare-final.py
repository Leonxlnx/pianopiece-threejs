...
b=json.loads(gzip.decompress((p/'final-baseline.json.gz').read_bytes()))
c=json.loads(gzip.decompress((p/sys.argv[1]).read_bytes()))
...
stem=sys.argv[2];(p/(stem+'-summary.json')).write_text(json.dumps(summary,indent=2));(p/(stem+'-details.json.gz')).write_bytes(gzip.compress(json.dumps({'keyRows':keyrows,'pairRows':pairrows,'contacts':contacts,'pointfail':pointfail,'targetHeld':targetheld}).encode()));print(json.dumps({k:v for k,v in summary.items() if k not in ['bySevereKey','byPair']},indent=2))
