...
function climaxRest(player, time) {
    const gaps = [[0, 0, 159.007812, 161.826373], [0, 2, 158.033095, 160.168206], [0, 3, 158.403465, 159.834873], [1, 0, 153.403546, 163.825895], [1, 2, 158.424465, 160.856873]];
    for (const [hi, fi, end, start] of gaps) {
        if (time <= end || time >= start)
            continue;
        const hand = player.hands[hi], f = hand.fingers[fi];
        if (hand.fingerNotes[fi].some(n => n.time <= time && n.time + n.duration > time))
            continue;
        const weight = smooth((time - end) / .22) * smooth((start - time) / .30), q = hand.wrist.getWorldQuaternion(new THREE.Quaternion()), origin = wp(f.bones[0]), world = [];
        if (fi === 0) {
            const reach = f.lengths.reduce((a, b) => a + b, 0), offset = v3(0.35 * (hi === 0 ? -1 : 1), 0.6, -0.12).multiplyScalar(reach).applyQuaternion(q), touch = origin.clone().add(offset);
            ...
            const spread = (fi === 2 ? 15 : 0) * (hi === 0 ? -1 : 1) * Math.PI / 180, normal = v3(-Math.cos(spread), Math.sin(spread), 0).applyQuaternion(q);
            for (let j = 0; j < 3; j++) {
                const angle = [-15, 30, 55][j] * Math.PI / 180, dir = v3(Math.sin(spread) * Math.cos(angle), Math.cos(spread) * Math.cos(angle), Math.sin(angle)).applyQuaternion(q);
                world.push(new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().makeBasis(normal, dir, normal.clone().cross(dir).normalize())).multiply(f.frameOffsets[j]));
            }
        }
...
export class Pianist extends OriginalPianist {
    ...
    update(time, score, piano, pedal, energy) { super.update(time, score, piano, pedal, energy); releaseRepair(this, time, piano); p299Idle(this, time); climaxRest(this, time); }
}
