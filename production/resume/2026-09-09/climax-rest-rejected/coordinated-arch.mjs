...
function curvedNeutral(player, hand, fi, fan) {
    const f = hand.fingers[fi], q = hand.wrist.getWorldQuaternion(new THREE.Quaternion()), origin = wp(f.bones[0]), world = [];
    if (fi === 0) {
        ...
        for (let j = 0; j < 3; j++) {
            const dir = p[j + 1].clone().sub(p[j]).normalize(), normal = chain.normal;
            world.push(new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().makeBasis(normal, dir, normal.clone().cross(dir).normalize())).multiply(f.frameOffsets[j]));
        }
    }
    else {
        const normal = v3(-Math.cos(fan), Math.sin(fan), 0).applyQuaternion(q), angles = [-25, 35, 65].map(a => a * Math.PI / 180);
        const dirs = (lift) => angles.map(angle => v3(Math.sin(fan) * Math.cos(angle - lift), Math.cos(fan) * Math.cos(angle - lift), Math.sin(angle - lift)).applyQuaternion(q));
        const clears = (lift) => { let point = origin.clone(); return dirs(lift).every((dir, j) => { point.addScaledVector(dir, f.lengths[j]); return point.y >= KEY_TOP + mix(.030, .017, smooth((point.z - .230) / .030)); }); };
        let low = 0, high = .85;
        if (clears(0))
            high = 0;
        else
        ...
        for (const [j, dir] of dirs(high).entries())
            world.push(new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().makeBasis(normal, dir, normal.clone().cross(dir).normalize())).multiply(f.frameOffsets[j]));
    }
    return world.map((rotation, j) => (j === 0 ? q : world[j - 1]).clone().invert().multiply(rotation));
}
function coordinatedRest(player, time) {
    for (const hand of player.hands) {
        const q = hand.wrist.getWorldQuaternion(new THREE.Quaternion()), inverse = q.clone().invert();
        let fanSum = 0, fanWeight = .35;
        ...
        for (let fi = 1; fi < 5; fi++) {
            const note = hand.fingerNotes[fi].find(n => n.time < time && n.time + n.duration > time);
            if (!note)
                continue;
            ...
            fanSum += clamp(Math.atan2(dir.x, dir.y), -.95, .95) * amount;
            fanWeight += amount;
        }
        ...
        for (let fi = 0; fi < 5; fi++) {
            ...
            if (start - end < .50 || end >= 160.5 || start <= 158.5 || time <= end || time >= start)
                continue;
...
export class Pianist extends OriginalPianist {
    ...
    update(time, score, piano, pedal, energy) { super.update(time, score, piano, pedal, energy); releaseRepair(this, time, piano); p299Idle(this, time); coordinatedRest(this, time); }
}
