...
function climaxRest(player, time) {
    ...
    for (const [hi, fi, end, start] of gaps) {
        ...
        if (fi === 0) {
            const reach = f.lengths.reduce((a, b) => a + b, 0), offset = v3(0.4 * (hi === 0 ? -1 : 1), 0.55, -0.1).multiplyScalar(reach).applyQuaternion(q), touch = origin.clone().add(offset);
            ...
            const spread = (fi === 2 ? 40 : 0) * (hi === 0 ? -1 : 1) * Math.PI / 180, normal = v3(-Math.cos(spread), Math.sin(spread), 0).applyQuaternion(q);
            for (let j = 0; j < 3; j++) {
                const angle = [-5, 40, 70][j] * Math.PI / 180, dir = v3(Math.sin(spread) * Math.cos(angle), Math.cos(spread) * Math.cos(angle), Math.sin(angle)).applyQuaternion(q);
                world.push(new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().makeBasis(normal, dir, normal.clone().cross(dir).normalize())).multiply(f.frameOffsets[j]));
            }
        }
