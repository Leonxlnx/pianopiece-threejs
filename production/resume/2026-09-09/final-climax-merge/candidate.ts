...
function rhOrderedArrival(player:OriginalPianist,time:number){
 const hand=player.hands[1];
 for(const c of rhOrderedArrivalControls){const keys=c.keys;if(time<=keys[0].time||time>=keys[keys.length-1].time)continue;
  if(hand.fingerNotes[c.fi].some(n=>n.time<=time&&n.time+n.duration>time))continue;
...
export class Pianist extends OriginalPianist {
 ...
 update(time:number,score:Score,piano:GrandPiano,pedal:number,energy:number){super.update(time,score,piano,pedal,energy);releaseRepair(this,time,piano);p299Idle(this,time);virtualChordRest(this,time,piano);liveChordTransitions(this,time,piano);climaxNonthumbContinuity(this,time);rhOrderedArrival(this,time);climaxThumbContinuity(this,time);relaxedClimaxThumb(this,time);}
}
