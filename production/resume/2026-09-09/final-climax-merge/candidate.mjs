import { applyIdleNonthumb } from '../daybreak-site/production/qa/compiled/idle-nonthumb.mjs';
import * as THREE from 'three';
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';
import { clamp, smooth, mix, v3, lowerBound } from '../daybreak-site/production/qa/compiled/math.mjs';
import { keyX, isBlack, KEY_TOP, keySurfaceY } from '../daybreak-site/production/qa/compiled/piano.mjs';
import { wristMotionSampler } from '../daybreak-site/production/qa/compiled/wrist-motion.mjs';
import { ponytailMotion } from '../daybreak-site/production/qa/compiled/ponytail-motion.mjs';
        ...
        this.model.traverse(o => {
            ...
            if (o.isMesh) {
                ...
                if (m.name === 'Human' && m.geometry.index) {
                    const old = m.geometry.index.array;
                    m.geometry = m.geometry.clone();
                    const pos = m.geometry.attributes.position, kept = [];
        ...
        for (const hand of this.hands) {
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                const solveFinger = (fnotes) => {
                    ...
                    if (!n && fi === 0) {
                        ...
                        if (hand.side === 'R' && previous?.id === 'p00112' && next?.id === 'p00142'
                            ...
                            const liftedContact = (note, at) => { const anchor = climaxContextAnchor(note, at, this.plannedPose(hand, at)), origin = finger.bones[0].position.clone().applyQuaternion(anchor.q).add(anchor.position), z = note.contactZ ?? this.contactDepth(origin.z, isBlack(note.midi), fi), touch = v3(keyX(note.midi), keySurfaceY(note.midi, z, 1) + (note.contactLift ?? .002) + .015, z); return localPose(anchor.position, anchor.q, touch, note.thumbOpposition ?? 0); };
                        ...
                        if (controls && previous && next && hand.side === 'R' && time > end && time < start) {
                            ...
                            const liftedContact = (note, at, lift) => { const anchor = climaxContextAnchor(note, at, this.plannedPose(hand, at)), origin = finger.bones[0].position.clone().applyQuaternion(anchor.q).add(anchor.position), z = note.contactZ ?? this.contactDepth(origin.z, isBlack(note.midi), fi), touch = v3(keyX(note.midi), keySurfaceY(note.midi, z, 1) + (note.contactLift ?? .002) + lift, z); return localPose(anchor.position, anchor.q, touch, note.thumbOpposition ?? 0); };
                    ...
                    if (!n && fi > 0) {
                        ...
                        const bassOriginalContacts = { "p00322": { "time": 71.178428, "duration": 0.269968, "contactZ": null, "contactLift": 0.0062824555694730895 } };
                        const boundEnvelope = (note, at, amount) => {
                            ...
                            if (!old || note.time !== old.time || note.duration !== old.duration)
                                return value;
                            const reference = { ...note, contactLift: old.contactLift };
                            if (old.contactZ === null)
                                delete reference.contactZ;
                            else
                                reference.contactZ = old.contactZ;
                            return mix(envelope(contactPose(reference, at)), value, amount);
                        };
                        if (previous)
                            weight = Math.min(weight, boundEnvelope(previous, end, previous && next && gap < .50 ? 1 - smooth((time - end) / Math.max(.001, gap)) : 1 - smooth((time - end) / .19)));
                        if (next) {
                            let nextEnvelope = boundEnvelope(next, start, previous && next && gap < .50 ? smooth((time - end) / Math.max(.001, gap)) : smooth(1 - (start - time) / .27));
                            if (hand.side === 'R' && fi === 1 && previous?.id === 'p00285' && next.id === 'p00302' && Math.abs(end - 64.184354) < 1e-7 && Math.abs(start - 67.016236) < 1e-7)
                                nextEnvelope = mix(.14897594070608658, nextEnvelope, smooth((time - 65.7) / .2));
                ...
                if (context && !current.some(n => n.finger === fi + 1)) {
                    const actual = finger.bones.map(b => b.quaternion.clone());
                    solveFinger(context.notes);
...
function releaseRepair(player, time, piano) {
    const envelope = smooth((time - 106.65) / .13) * (1 - smooth((time - 107.35) / .20));
    if (envelope === 0)
        return;
    const hand = player.hands[1];
    for (let fi = 1; fi < 5; fi++) {
        if (hand.notes.some(n => n.finger === fi + 1 && n.time <= time && n.time + n.duration > time))
            continue;
        const f = hand.fingers[fi], ns = hand.fingerNotes[fi];
        let previous, next;
        for (const n of ns) {
        ...
        if (fi === 1 && previous && next && ['p00519', 'p00524'].includes(previous.id)) {
            const end = previous.time + previous.duration, gap = next.time - end, u = (time - end) / gap, a = contact(previous, end), b = contact(next, next.time);
            rotations = a.map((q, j) => q.slerp(b[j], smooth(u)));
            rotations[0].premultiply(new THREE.Quaternion().setFromAxisAngle(v3(-1, 0, 0), Math.sin(u * Math.PI) ** 2 * (0) * Math.PI / 180));
            ...
            const w = smooth(since / .19) * smooth(before / .27);
            const neutral = f.rest.map(q => q.clone()), wristQ = hand.wrist.getWorldQuaternion(new THREE.Quaternion()), axis = v3(-1, 0, 0).applyQuaternion(wristQ);
            let parentQ = wristQ.clone();
            for (let j = 0; j < 3; j++) {
                const angle = [30, -(35), -(20)][j], localAxis = axis.clone().applyQuaternion(parentQ.clone().invert());
                neutral[j].premultiply(new THREE.Quaternion().setFromAxisAngle(localAxis, angle * Math.PI / 180));
                parentQ.multiply(neutral[j]);
            }
            rotations.forEach((q, j) => q.copy(f.bones[j].quaternion).slerp(neutral[j], w));
        }
...
function p299Idle(player, time) {
    ...
    for (const fi of [1, 3]) {
        if (hand.notes.some(n => n.finger === fi + 1 && n.time <= time && n.time + n.duration > time))
            continue;
        let previous, next;
        for (const n of hand.fingerNotes[fi]) {
            if (n.time + n.duration <= time)
                previous = n;
            else if (n.time >= time) {
                next = n;
        ...
        if (!previous || !next)
            continue;
        let amount = 0;
        if (fi === 1 && previous.id === 'p00285' && next.id === 'p00302')
            amount = 10 * p299Ease((time - 65.9) / .13) * p299Ease((next.time - time) / .27);
        if (fi === 3 && previous.id === 'p00296' && next.id === 'p00301')
            amount = 15 * p299Ease((time - previous.time - previous.duration) / 0.1) * p299Ease((next.time - time) / 0.1);
        if (!amount)
            continue;
        ...
        bone.quaternion.premultiply(parentQ.clone().invert().multiply(worldQ).multiply(parentQ));
        bone.updateWorldMatrix(false, true);
    }
}
function virtualChordRest(player, time, piano) {
    ...
    for (const scope of scopes) {
        const amount = smooth((time - scope.begin) / (scope.arrival - scope.begin)) * (1 - smooth((time - scope.end) / (scope.finish - scope.end)));
        if (!amount)
            continue;
        const hand = player.hands[scope.hi], q = hand.wrist.getWorldQuaternion(new THREE.Quaternion());
        for (const [fi, midi] of scope.lanes) {
            const notes = hand.fingerNotes[fi];
            if (notes.some(n => n.time <= time && n.time + n.duration > time))
                continue;
            ...
            const weight = amount * smooth(since / .09) * smooth(before / .12), f = hand.fingers[fi], base = wp(f.bones[0]), reach = f.lengths.reduce((a, b) => a + b, 0), z = base.z - reach * .75, touch = piano.contact(midi, z);
            touch.y += .024;
            if (scope.hi === 1) {
                if (time > 159.188206)
                    touch.x += wp(hand.wrist).x - .0796966666666667;
                if (fi === 1)
                    touch.x -= .006;
            }
            else if (time > 159.159706)
                touch.x += wp(hand.wrist).x + .218;
            ...
            f.bones.forEach((bone, j) => { bone.quaternion.slerp(local[j], weight); bone.updateWorldMatrix(false, true); });
        }
    }
}
function liveChordTransitions(player, time, piano) {
    if (time < 158.95 || time > 159.85)
        return;
    const touched = new Set(['p00719', 'p00720', 'p00721', 'p00722', 'p00723']), arrivals = new Set([...touched, 'p00724', 'p00725']);
    for (const hand of player.hands) {
        const q = hand.wrist.getWorldQuaternion(new THREE.Quaternion());
        for (let fi = 0; fi < 5; fi++) {
            const notes = hand.fingerNotes[fi];
            if (notes.some(n => n.time <= time && n.time + n.duration > time))
                continue;
            const previous = notes.filter(n => n.time + n.duration <= time).at(-1), next = notes.find(n => n.time >= time), since = previous ? time - previous.time - previous.duration : 100, before = next ? next.time - time : 100;
            const incoming = next && arrivals.has(next.id) && before < .20, outgoing = previous && touched.has(previous.id) && since < .18;
            if (!incoming && !outgoing)
                continue;
            const f = hand.fingers[fi], base = wp(f.bones[0]), reach = f.lengths.reduce((a, b) => a + b, 0), touchOf = (n) => { const z = n.contactZ ?? player.contactDepth(base.z, isBlack(n.midi), fi), p = piano.contact(n.midi, z); p.y += n.contactLift ?? .002; return p; };
            let touch, amount, opposition = 0;
            if (incoming && previous && next.time - previous.time - previous.duration < .50) {
                const gap = next.time - previous.time - previous.duration, u = clamp(since / gap);
                touch = touchOf(previous).lerp(touchOf(next), smooth(u));
                touch.y += .030 * Math.sin(u * Math.PI) ** 2;
                amount = 1;
                opposition = mix(previous.thumbOpposition ?? 0, next.thumbOpposition ?? 0, smooth(u));
            }
            else if (incoming) {
                touch = touchOf(next);
                touch.y += .020 * smooth(before / .12);
                amount = smooth((.20 - before) / .08);
                opposition = next.thumbOpposition ?? 0;
            }
            else {
                const lanes = hand.side === 'L' ? [55, 50, 47, 45, 43] : [67, 69, 71, 72, 74], z = base.z - reach * .75, neutral = piano.contact(lanes[fi], z);
                neutral.y += .024;
                neutral.x += wp(hand.wrist).x - (hand.side === 'L' ? -.218 : .0796966666666667);
                if (hand.side === 'R' && fi === 1)
                    neutral.x -= .006;
                const u = clamp(since / .09);
                touch = touchOf(previous).lerp(neutral, smooth(u));
                touch.y += .006 * Math.sin(u * Math.PI) ** 2;
                amount = (1 - smooth((since - .11) / .07)) * smooth(before / .07);
                opposition = previous.thumbOpposition ?? 0;
            }
...
function climaxIdleContext(hand, fi, time) {
    ...
    notes.push(...climaxOriginalIdleNotes.filter(n => n.hand === hand.side && n.finger === fi + 1));
...
function climaxNonthumbContinuity(player, time) {
    if (time <= 158.99 || time >= 159.153206)
        return;
    const hand = player.hands[0], u = smooth((time - 158.99) / (159.153206 - 158.99));
    for (const fi of [1, 2, 3]) {
        if (hand.fingerNotes[fi].some(n => n.time <= time && n.time + n.duration > time))
            continue;
        hand.fingers[fi].bones.forEach((b, j) => { b.quaternion.fromArray(climaxNonthumbArrival[0][fi][j]).slerp(new THREE.Quaternion().fromArray(climaxNonthumbArrival[1][fi][j]), u); if (j === 0)
            b.quaternion.premultiply(new THREE.Quaternion().setFromAxisAngle(v3(...climaxNonthumbLiftAxes[fi - 1]), .10 * Math.sin(Math.PI * (time - 158.99) / (159.153206 - 158.99)) ** 2)); b.updateWorldMatrix(false, true); });
    }
}
// Keep the waiting Pinky clear of the Ring-held D5 lane. Every control
// is a complete local joint posture; active contacts bypass this curve.
const rhOrderedArrivalControls = [{ "fi": 3, "keys": [{ "time": 159.157206, "q": [[0.05308168066748613, -0.16502704442185318, 0.08162137975622426, 0.9814715279383264], [-0.3919313653894685, 0.11006023472254448, 0.1119942366648432, -0.9064953615494259], [0.18676993378812043, 0.030629087976703134, -0.07198782746846986, 0.9792837196128947]] }, { "time": 159.188206, "q": [[-0.24266998079680285, -0.13000710992282966, -0.11416375320953007, 0.9545554352664807], [0.48976857965029724, -0.1037197887864201, -0.13995124383
0.0015330425644796593, -0.14736059352746544, 0.771224066362764], [-0.30772896694631224, 0.0920942847990818, 0.10511756867199076, -0.9411545156699779]] }, { "time": 159.157206, "q": [[-0.0861046514486227, 0.42564157905044325, -0.09466599658545433, -0.895797751727652], [-0.35531199510496914, -0.001850867426634678, 0.08455030379591791, -0.9309141794437787], [-0.16891383858969258, 0.09582339732124309, 0.05769935101913702, -0.9792633849849872]] }, { "time": 159.188206, "q": [[-0.28063031141137557, 0.21917868142575894, 0.10184418212206889, -0.9288891786399995], [-0.35531199510496914, -0.001850867426634678, 0.08455030379591791, -0.9309141794437787], [-0.16891383858969258, 0.09582339732124309, 0.05769935101913702, -0.9792633849849872]] }] }];
function rhOrderedArrival(player, time) {
    const hand = player.hands[1];
    for (const c of rhOrderedArrivalControls) {
        const keys = c.keys;
        if (time <= keys[0].time || time >= keys[keys.length - 1].time)
            continue;
        if (hand.fingerNotes[c.fi].some(n => n.time <= time && n.time + n.duration > time))
            continue;
        const b = keys.findIndex(k => k.time >= time), a = keys[b - 1], z = keys[b], u = smooth((time - a.time) / (z.time - a.time));
        hand.fingers[c.fi].bones.forEach((bone, j) => { bone.quaternion.fromArray(a.q[j]).slerp(new THREE.Quaternion().fromArray(z.q[j]), u); bone.updateWorldMatrix(false, true); });
    }
}
...
function climaxThumbContinuity(player, time) {
    for (const c of climaxThumbCurves) {
        const first = c.knots[0], last = c.knots[c.knots.length - 1];
        if (time < first.time || time > last.time)
            continue;
        const hand = player.hands.find(h => h.side === c.side);
        const notes = hand.fingerNotes[0];
        if (notes.some(n => n.time <= time && n.time + n.duration > time))
            continue;
        const previous = notes.filter(n => n.time + n.duration <= time).at(-1), next = notes.find(n => n.time >= time);
        if (previous?.id !== c.previous || next?.id !== c.next || Math.abs(previous.time + previous.duration - c.previousEnd) > 1e-7 || Math.abs(next.time - c.nextTime) > 1e-7)
            continue;
        let index = 1;
        while (index < c.knots.length - 1 && time > c.knots[index].time)
            index++;
...
function relaxedClimaxThumb(player, time) {
    if (time <= 159.5 || time >= 163.525895)
        return;
    ...
    if (previous?.id !== 'p00721' || next?.id !== 'p00750' || Math.abs(previous.time + previous.duration - 159.38365) > 1e-7 || Math.abs(next.time - 163.825895) > 1e-7)
        return;
...
export class Pianist extends OriginalPianist {
    plannedPose(hand, time) { const pose = super.plannedPose(hand, time); if (hand.side === 'L' && time > 159.374607 && time < 159.498539) {
        pose.position.y += .005 * Math.sin(Math.PI * (time - 159.374607) / (159.498539 - 159.374607)) ** 2;
    } if (hand.side === 'R') {
        const w = p299Ease((time - 66.18) / (66.30195 - 66.18)) * (1 - p299Ease((time - 66.54) / (66.668093 - 66.54)));
        pose.position.x -= .035 * w;
        for (const [end, start, dy, dz] of [[106.81, 106.919774, .020, .024], [107.18, 107.269683, .024, 0]]) {
            if (time <= end || time >= start)
                continue;
            const u = (time - end) / (start - end), w = Math.sin(u * Math.PI) ** 2;
            pose.position.y += dy * w;
            pose.position.z += dz * w;
        }
    } return pose; }
    update(time, score, piano, pedal, energy) { super.update(time, score, piano, pedal, energy); releaseRepair(this, time, piano); p299Idle(this, time); virtualChordRest(this, time, piano); liveChordTransitions(this, time, piano); climaxNonthumbContinuity(this, time); rhOrderedArrival(this, time); climaxThumbContinuity(this, time); relaxedClimaxThumb(this, time); }
}
