...
const {Pianist}=await import(process.env.DAYBREAK_RIG_MODULE?pathToFileURL(process.env.DAYBREAK_RIG_MODULE).href:'./baseline.mjs');
import {GrandPiano,keyX,isBlack,keySurfaceY} from '/workspace/scratch/2e8cc8e77f98/climax-nonthumb-continuity/render-project/production/qa/compiled/piano.mjs';
import {pedalPosition,scoreEnergy} from '/workspace/scratch/2e8cc8e77f98/climax-nonthumb-continuity/render-project/production/qa/compiled/math.mjs';
const {wristMotionSampler}=await import(process.env.DAYBREAK_WRIST_MODULE?pathToFileURL(process.env.DAYBREAK_WRIST_MODULE).href:'/workspace/scratch/2e8cc8e77f98/climax-nonthumb-continuity/render-project/production/qa/compiled/wrist-motion.mjs');
...
export const baseline=JSON.parse(fs.readFileSync(process.env.DAYBREAK_SCORE??new URL('./input.json',import.meta.url)));
