        ...
        for (const hand of this.hands) {
            ...
            for (let fi = 0; fi < 5; fi++) {
                ...
                if (!n && fi > 0) {
                    ...
                    if(hand.side==='R'&&fi===1&&time>=64.15&&time<=67.02)this.p299Trace={time,previous:previous?.id,next:next?.id,weight,idle:envelope(idlePose),rotations:envelope(rotations),previousCap:previous?envelope(contactPose(previous,end)):null,nextCap:next?envelope(contactPose(next,start)):null};
