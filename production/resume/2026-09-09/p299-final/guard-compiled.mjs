...
class OriginalPianist {
    ...
    async load(score, preparedModel) {
        for (const [id, end] of [["p00296", 66.18], ["p00299", 66.54]]) {
            const n = score.notes.find(n => n.id === id);
            n.writtenDuration ??= n.duration;
            n.duration = Number(end) - n.time;
        }
        Object.assign(score.notes.find(n => n.id === "p00299"), { finger: 5, contactZ: .270, contactLift: .003 });
        this.score = score;
...
function p299Idle(player, time) {
    ...
    for (const fi of [1, 3]) {
        ...
        if (fi === 3 && previous.id === 'p00296' && next.id === 'p00301')
            amount = 15 * p299Ease((time - previous.time - previous.duration) / .10) * p299Ease((next.time - time) / .10);
        if (!amount)
            continue;
