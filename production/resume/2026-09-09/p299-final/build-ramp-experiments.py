...
function p299Idle(player,time,piano){
 const hand=player.hands[1],config=player.repairConfig??{lift:10,ringLift:10,ringRamp:.10};
 ...
 for(const fi of [1,3]){
  ...
  if(config.lift!==undefined){
   ...
   if(fi===3&&previous.id==='p00296'&&next.id==='p00301')amount=config.ringLift*phraseEase((time-previous.time-previous.duration)/config.ringRamp)*phraseEase((next.time-time)/config.ringRamp);
