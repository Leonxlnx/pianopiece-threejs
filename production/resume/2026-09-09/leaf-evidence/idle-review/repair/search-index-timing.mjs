...
import {idleNonthumbData as data} from './search-timing-candidate.mjs';
const base=await import('../harness.mjs?rig=./baseline.mjs'),test=await import('../harness.mjs?rig=./repair/search-timing-candidate.mjs');
...
const times=new Set();for(let f=Math.ceil(175.4*240);f<=Math.floor(176.25*240);f++)times.add(f/240);
...
const variants=[.4,.5,.6,.7,.8,1,1.5,2,4].map(arrivalFadeSeconds=>({name:`arrivalFade${arrivalFadeSeconds}`,arrivalFadeSeconds}));
const output=[];
for(const variant of variants){data.supportedIndexGaps.splice(0,data.supportedIndexGaps.length,...structuredClone(original.supportedIndexGaps));data.supportedIndexGaps[58].arrivalFadeSeconds=variant.arrivalFadeSeconds;
...
fs.writeFileSync(new URL('./search-index-timing-results.json',import.meta.url),JSON.stringify(output,null,2)+'\n');
