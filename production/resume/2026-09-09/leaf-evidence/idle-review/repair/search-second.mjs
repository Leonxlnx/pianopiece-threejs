...
const variants=[];
for(const lift of [0,3,6,8,9.5,12])for(const spread of [-14,-16,-20,-24])variants.push({name:`lift${lift}-spread${spread}`,lift,spread});
for(const pip of [-10,-5,3,6,9,12])for(const dip of [-5,0,5])variants.push({name:`pip${pip}-dip${dip}`,lift:9.5,spread:-13,pip,dip});
const output=[];
for(const variant of variants){data.curves.splice(0,data.curves.length,...structuredClone(original.curves));if(variant.remove)data.curves.splice(6,1);else for(const k of data.curves[6].knots)if(k[1]){k[1]=variant.lift;k[2]=variant.spread;k[3]=variant.pip??0;k[4]=variant.dip??0;}
...
fs.writeFileSync(new URL('./search-second-results.json',import.meta.url),JSON.stringify(output,null,2)+'\n');
