...
const times=new Set();for(let f=Math.ceil(91.40*480);f<=Math.floor(91.54*480);f++)times.add(f/480);
for(const t of [91.40,91.54,...target.knots.map(x=>x[0]),...base.score.notes.flatMap(n=>[n.time,n.time+n.duration]).filter(t=>t>=91.40&&t<=91.54)])for(const d of [-1e-6,0,1e-6])times.add(t+d);
...
const variants=[];
for(const lift of [13,15,18])for(const spread of [-18,-20,-22,-24])for(const timing of [[91.46,91.474,91.482,91.518],[91.445,91.474,91.482,91.518],[91.435,91.474,91.49,91.522]])variants.push({name:`lift${lift}-spread${spread}-begin${timing[0]}`,lift,spread,timing});
for(const scale of [.1,.25,.5,.75])variants.push({name:`scale${scale}`,lift:9.5*scale,spread:-13*scale});
const output=[];
for(const variant of variants){data.curves.splice(0,data.curves.length,...structuredClone(original.curves));if(variant.remove)data.curves.splice(6,1);else for(const k of data.curves[6].knots)if(k[1]){k[1]=variant.lift;k[2]=variant.spread;}if(variant.timing)data.curves[6].knots.forEach((k,i)=>k[0]=variant.timing[i]);
...
fs.writeFileSync(new URL('./search-third-results.json',import.meta.url),JSON.stringify(output,null,2)+'\n');
