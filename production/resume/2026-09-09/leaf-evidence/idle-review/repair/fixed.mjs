...
function applyIdleNonthumb(time, side, fi, bones, wrist, base, previous, next) {
    ...
    if (fi === 1) {
        ...
        if (c) {
            const amount = idleSmooth((base.y - KEY_TOP - .005) / .004) * idleSmooth((base.z - .244) / .008) * idleSmooth((time - c.previousEnd) / .25) * idleSmooth((c.nextTime - time) / .32) * (c.earlyRelease ? 1 - idleSmooth((time - c.earlyRelease[0]) / (c.earlyRelease[1] - c.earlyRelease[0])) : 1);
