...
import { clamp, segment, v3 } from './math.mjs';
const WHITE_WIDTH = .0235;
export const KEY_TOP = .742;
export const KEY_FRONT = .28;
export function isBlack(midi) { return [1, 3, 6, 8, 10].includes(midi % 12); }
export function keyX(midi) { let count = 0; for (let m = 21; m < midi; m++)
    if (!isBlack(m))
        count++; return -.611 + count * WHITE_WIDTH + (isBlack(midi) ? -.0015 : WHITE_WIDTH * .5); }
// Top surface of the actual hinged key at a requested front/back contact depth.
export function keySurfaceY(midi, z, down = 1) { const black = isBlack(midi), angle = down * (black ? .028 : .033), half = black ? .0115 : .008, pivotY = KEY_TOP + (black ? .012 : -.008), localZ = (z + .002 - half * Math.sin(angle)) / Math.cos(angle); return pivotY + half * Math.cos(angle) - localZ * Math.sin(angle); }
...
function slab(shape, depth, mat, y, parent) { const geo = new THREE.ExtrudeGeometry(shape, { depth, steps: 1, bevelEnabled: true, bevelSize: .007, bevelThickness: .006, bevelSegments: 3, curveSegments: 48 }); geo.rotateX(-Math.PI / 2); const mesh = new THREE.Mesh(geo, mat); mesh.position.y = y; mesh.castShadow = true; mesh.receiveShadow = true; parent.add(mesh); return mesh; }
function box(parent, w, h, d, x, y, z, mat, r = .005) { const m = new THREE.Mesh(new RoundedBoxGeometry(w, h, d, 2, r), mat); m.position.set(x, y, z); m.castShadow = true; m.receiveShadow = true; parent.add(m); return m; }
function woodTexture(pending) {
...
function batchStaticPiano(root, protectedRoots) {
    const protectedObjects = new Set();
    for (const item of protectedRoots)
        item.traverse(o => protectedObjects.add(o));
    root.updateMatrixWorld(true);
    const inverseRoot = root.matrixWorld.clone().invert();
    const buckets = new Map(), sourceOrder = new Map();
    root.traverseVisible(object => {
        const mesh = object;
        if (!mesh.isMesh || mesh.isInstancedMesh || mesh.isSkinnedMesh || protectedObjects.has(mesh) || Array.isArray(mesh.material))
            return;
        const material = mesh.material, geometry = mesh.geometry;
        if (material.transparent || !material.visible || mesh.morphTargetInfluences || geometry.drawRange.start !== 0 || geometry.drawRange.count !== Infinity || mesh.matrixWorld.determinant() <= 0)
            return;
        ...
        sourceOrder.set(mesh, sourceOrder.size);
    ...
    root.traverse(object => { const mesh = object; if (mesh.geometry)
        removedGeometries.delete(mesh.geometry); });
...
export class GrandPiano {
    ...
    damperStems;
    damperRest = [];
    damperDummy = new THREE.Object3D();
    active = [];
    whiteKeyBatch;
    blackKeyBatch;
    keyMatrix = new THREE.Matrix4();
    constructor() {
        ...
        const rim = outline();
        const hole = outline(.035);
        rim.holes.push(new THREE.Path(hole.getPoints(72)));
        slab(rim, .2, lacquer, .745, this.group);
        // Cast iron plate, bridges and belly braces.
        const plate = outline(.085);
        const hole2 = outline(.20);
        plate.holes.push(new THREE.Path(hole2.getPoints(72)));
        slab(plate, .026, gold, .83, this.group);
        // Cast webs connect to the inner plate border and remain below the wires.
        const plateOpening = outline(.20).getPoints(160);
        const openingEdges = (z) => {
            const y = -z, hits = [];
            for (let j = 0; j < plateOpening.length; j++) {
                const a = plateOpening[j], b = plateOpening[(j + 1) % plateOpening.length];
        ...
        for (let i = 0; i < 4; i++) {
            const z = -.47 - i * .43, a = new THREE.Vector2(openingEdges(z)[0] - .028, z), b = new THREE.Vector2(openingEdges(z - .085)[1] + .028, z - .085), axis = b.clone().sub(a).normalize(), normal = new THREE.Vector2(-axis.y, axis.x), points = [];
            for (const sign of [-1, 1]) {
                const row = [a.clone().addScaledVector(normal, sign * .047), a.clone().addScaledVector(axis, .080).addScaledVector(normal, sign * .026), b.clone().addScaledVector(axis, -.080).addScaledVector(normal, sign * .026), b.clone().addScaledVector(normal, sign * .047)];
                points.push(...(sign === 1 ? row.reverse() : row));
            }
            const shape = new THREE.Shape();
            points.forEach((p, j) => j ? shape.lineTo(p.x, -p.y) : shape.moveTo(p.x, -p.y));
            shape.closePath();
            const g = new THREE.ExtrudeGeometry(shape, { depth: .0175, steps: 1, bevelEnabled: true, bevelSize: .0035, bevelThickness: .004, bevelSegments: 3 });
            g.rotateX(-Math.PI / 2);
            const web = new THREE.Mesh(g, gold);
            web.name = 'Connected cast plate web ' + i;
            web.position.y = .829;
            web.castShadow = true;
        ...
        for (const bass of [true, false]) {
            ...
            const bridge = new THREE.Mesh(new THREE.TubeGeometry(curve, 100, bass ? .025 : .019, 8, false), wood);
            bridge.name = bass ? 'Raised bass bridge' : 'Tenor and treble bridge';
            bridge.castShadow = true;
            this.group.add(bridge);
        }
        const steelParts = [], copperParts = [], windings = [], pinPositions = [], hitchPositions = [];
        const cylinderBetween = (a, b, r) => {
        ...
        for (const c of courses) {
            ...
            for (let j = 0; j < c.copies; j++) {
                ...
                const pin = a.clone().addScaledVector(direction, -.045 - j * .015);
                pinPositions.push(pin);
                hitchPositions.push(hitch);
                // Wound bass detail is actual small geometry, not a thick luminous line.
                if (c.bass) {
                    const length = a.distanceTo(b), count = Math.ceil(length / .0042), up = direction.clone().cross(lateral).normalize();
                    for (let k = 0; k < count; k++) {
                        ...
                        const p2 = center.clone().addScaledVector(direction, .0022).addScaledVector(lateral, -Math.cos(phase) * .00123).addScaledVector(up, -Math.sin(phase) * .00123);
                        windings.push(...p1.toArray(), ...p2.toArray());
                    }
                }
            }
            if (c.midi <= 87) {
                const p = c.start.clone().lerp(c.end, .16);
                this.damperRest.push({ midi: c.midi, p, angle: Math.atan2(-direction.x, -direction.z) });
            }
        }
        for (const [parts, mat, name] of [[steelParts, new THREE.MeshStandardMaterial({ color: 0xa9b0ac, metalness: .95, roughness: .25 }), 'Individual steel strings'], [copperParts, new THREE.MeshStandardMaterial({ color: 0xa8734e, metalness: .93, roughness: .39 }), 'Copper-wound bass strings']]) {
        ...
        const windingGeometry = new THREE.BufferGeometry();
        windingGeometry.setAttribute('position', new THREE.Float32BufferAttribute(windings, 3));
        const windingMesh = new THREE.LineSegments(windingGeometry, new THREE.LineBasicMaterial({ color: 0x77563c, transparent: true, opacity: .50 }));
        windingMesh.name = 'Bass winding ridges';
        this.group.add(windingMesh);
        ...
        for (let i = 0; i < pinPositions.length; i++) {
            dummy.position.copy(pinPositions[i]).add(v3(0, .004, 0));
            dummy.updateMatrix();
            pins.setMatrixAt(i, dummy.matrix);
            dummy.position.copy(hitchPositions[i]).add(v3(0, -.001, 0));
            dummy.updateMatrix();
            hitches.setMatrixAt(i, dummy.matrix);
        }
        pins.name = 'Tuning pin for each string';
        hitches.name = 'Hitch pins beyond each bridge';
        pins.castShadow = true;
        this.group.add(pins, hitches);
        ...
        this.damperCaps.name = 'Moving damper heads';
        this.damperFelts.name = 'Moving damper felts';
        this.damperStems.name = 'Damper wires';
        for (const mesh of [this.damperCaps, this.damperFelts, this.damperStems]) {
        ...
        for (let i = 0; i < 19; i++) {
            ...
            const washer = new THREE.Mesh(new THREE.CylinderGeometry(.010, .010, .0025, 16), gold);
            washer.position.set(x, .862, z);
            this.group.add(washer);
            const bolt = new THREE.Mesh(new THREE.CylinderGeometry(.006, .006, .005, 6), brass);
            bolt.position.set(x, .866, z);
            this.group.add(bolt);
        }
        ...
        for (let midi = 21; midi <= 108; midi++) {
            ...
            pivot.position.set(x, KEY_TOP - (black ? -.012 : .008), -.002);
            ...
            mesh.position.set(0, 0, black ? .179 : .1395);
            mesh.visible = false;
            mesh.castShadow = true;
        ...
        this.whiteKeyBatch.name = '52 independently moving white keys';
        this.blackKeyBatch.name = '36 independently moving black keys';
        for (const batch of [this.whiteKeyBatch, this.blackKeyBatch]) {
        ...
        box(this.group, .08, .128, .372, -.694, .728, .10, lacquer, .012);
        box(this.group, .08, .128, .372, .694, .728, .10, lacquer, .012);
        ...
        ct.clearRect(0, 0, 512, 96);
        ct.fillStyle = '#bc9e66';
        ct.font = '32px Georgia';
        ct.textAlign = 'center';
        ct.fillText('D A Y B R E A K', 256, 55);
        const tex = new THREE.CanvasTexture(c);
        tex.colorSpace = THREE.SRGBColorSpace;
        const label = new THREE.Mesh(new THREE.PlaneGeometry(.205, .039), new THREE.MeshBasicMaterial({ map: tex, transparent: true, depthWrite: false }));
        label.position.set(0, .829, .1292);
        this.group.add(label);
        // Raised lid rotates around the bass rim; soundboard remains physically open.
        const lidPivot = new THREE.Group();
        lidPivot.position.set(-.75, .963, 0);
        this.group.add(lidPivot);
        const lid = slab(outline(), .035, lacquer, 0, lidPivot);
        lid.position.x = .75;
        lidPivot.rotation.z = .44;
        ...
        const lidInside = slab(outline(.034), .009, lidFinish, -.006, lidPivot);
        lidInside.position.x = .75;
        ...
        for (const z of [-.4, -1.65])
            box(this.group, .04, .014, .075, -.752, .956, z, brass, .002);
        // Tapered grand legs and brass twin casters.
        for (const [x, z] of [[-.637, .015], [.637, .015], [-.29, -2.05]]) {
            const leg = new THREE.Mesh(new THREE.CylinderGeometry(.036, .055, .565, 5), lacquer);
            leg.position.set(x, .36, z);
            leg.castShadow = true;
            this.group.add(leg);
            box(this.group, .108, .071, .112, x, .638, z, lacquer, .01);
            box(this.group, .06, .061, .055, x, .066, z, brass, .005);
            for (const dx of [-.026, .026]) {
                const caster = new THREE.Mesh(new THREE.CylinderGeometry(.031, .031, .012, 20), brass);
                caster.rotation.z = Math.PI / 2;
                caster.position.set(x + dx, .034, z);
                this.group.add(caster);
            }
        }
        // Pedal lyre and three separate hinged pedals, including moving damper rod.
        segment(this.group, v3(-.13, .67, -.09), v3(-.10, .105, .00), .017, lacquer);
        segment(this.group, v3(.13, .67, -.09), v3(.10, .105, .00), .017, lacquer);
        box(this.group, .292, .055, .095, 0, .116, .005, lacquer, .009);
        for (let i = 0; i < 3; i++) {
            const g = new THREE.Group();
            g.position.set((i - 1) * .072, .070, .025);
            const p = box(g, .043, .013, .142, 0, 0, .064, brass, .005);
            this.pedals.push(g);
            this.group.add(g);
            segment(this.group, v3((i - 1) * .072, .115, -.01), v3((i - 1) * .072, .61, -.05), .004, brass, .004, 7);
        }
        ...
        for (let i = 0; i < 3; i++)
            for (let j = 0; j < 2; j++) {
                const button = new THREE.Mesh(new THREE.SphereGeometry(.006, 10, 5), edge);
                button.scale.y = .22;
                button.position.set((i - 1) * .158, .498, .685 + j * .15);
                this.group.add(button);
            }
        box(this.group, .588, .035, .318, 0, .413, .76, lacquer, .006);
        for (const x of [-.251, .251])
            for (const z of [.632, .888]) {
                segment(this.group, v3(x, .045, z), v3(x, .419, z), .019, lacquer, .023, 7);
                box(this.group, .043, .027, .043, x, .019, z, brass, .004);
            }
        batchStaticPiano(this.group, [...this.pedals, ...[...this.keys.values()].map(key => key.pivot)]);
    }
    update(time, notes, pedal) {
        this.active = [];
        for (const key of this.keys.values())
            key.pivot.rotation.x = 0;
        for (const n of notes) {
            const t = time - n.time, travel = .048 - .031 * clamp((n.velocity - .3) / .6);
            if (t < -travel || t > n.duration + .10)
                continue;
            const key = this.keys.get(n.midi);
            if (!key)
                continue;
            const down = t < 0 ? clamp(1 + t / travel) : t < n.duration ? 1 : 1 - clamp((t - n.duration) / .10);
            key.pivot.rotation.x = Math.max(key.pivot.rotation.x, down * (key.black ? .028 : .033));
            if (t >= 0 && t < n.duration)
                this.active.push(n.midi);
        }
        this.pedals[2].rotation.x = pedal * .16;
        this.updateDampers(pedal);
        this.updateKeyInstances();
    }
    updateKeyInstances() {
        if (!this.whiteKeyBatch || !this.blackKeyBatch)
            return;
        let white = 0, black = 0;
        for (const key of this.keys.values()) {
            key.pivot.updateMatrix();
            key.mesh.updateMatrix();
            this.keyMatrix.multiplyMatrices(key.pivot.matrix, key.mesh.matrix);
            (key.black ? this.blackKeyBatch : this.whiteKeyBatch).setMatrixAt(key.black ? black++ : white++, this.keyMatrix);
        }
        this.whiteKeyBatch.instanceMatrix.needsUpdate = true;
        this.blackKeyBatch.instanceMatrix.needsUpdate = true;
    }
    updateDampers(pedal) {
        if (!this.damperCaps || !this.damperFelts || !this.damperStems)
            return;
        for (let i = 0; i < this.damperRest.length; i++) {
            ...
            const o = this.damperDummy;
            o.rotation.set(0, d.angle, 0);
            o.position.copy(d.p).add(v3(0, .015 + lift, 0));
            o.updateMatrix();
            this.damperCaps.setMatrixAt(i, o.matrix);
            o.position.y -= .008;
            o.updateMatrix();
            this.damperFelts.setMatrixAt(i, o.matrix);
            o.position.copy(d.p).add(v3(0, -.040 + lift, .018));
            o.updateMatrix();
            this.damperStems.setMatrixAt(i, o.matrix);
        }
        for (const mesh of [this.damperCaps, this.damperFelts, this.damperStems])
            mesh.instanceMatrix.needsUpdate = true;
    }
    contact(midi, depth) { const k = this.keys.get(midi), z = depth ?? (k.black ? .207 : .251), down = k.pivot.rotation.x / (k.black ? .028 : .033); return v3(k.x, keySurfaceY(midi, z, down), z); }
