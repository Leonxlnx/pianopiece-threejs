...
const {Pianist}=await import(new URL(new URL(import.meta.url).searchParams.get('rig'), import.meta.url).href);
import {GrandPiano,keyX,isBlack,keySurfaceY} from './compiled/piano.mjs';
import {pedalPosition,scoreEnergy} from './compiled/math.mjs';
const {wristMotionSampler}=await import('./compiled/wrist-motion.mjs');
...
const bytes=fs.readFileSync(new URL('./sources/pianist.glb',import.meta.url));
const gltf=await loader.parseAsync(bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength),'');
export const baseline=JSON.parse(fs.readFileSync(new URL('./sources/score-v11.json',import.meta.url)));
...
export function measure(time,side,{exact=true,allDigits=true,targetFinger=null,opposing=false,alreadyUpdated=false}={}){
 if(!alreadyUpdated)update(time);const offset=side==='L'?0:6,owners=Array.from({length:6},(_,i)=>offset+i),active=score.notes.filter(n=>n.hand===side&&n.time<=time&&n.time+n.duration>time),activeIndices=active.map(n=>offset+n.finger-1);
