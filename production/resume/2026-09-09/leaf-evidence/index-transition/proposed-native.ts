...
class OriginalPianist {
 ...
 for(const [id,end]of [["p00519",106.81],["p00524",107.18]]){const n=score.notes.find(n=>n.id===id)!;n.writtenDuration??=n.duration;n.duration=Number(end)-n.time;}
 this.score=score;
 ...
 const controls=[{"name":"g95","previous":"p00451","next":"p00459","end":95.039182,"start":95.94507,"target":[0.2,0.45,-0.1],"release":0.22,"arrival":0.22,"ramp":0.1,"arrivalRamp":0.04,"releaseLift":0.03,"arrivalLift":0.03}].find((c:any)=>c.previous===previous?.id&&c.next===next?.id&&Math.abs(end-c.end)<1e-7&&Math.abs(start-c.start)<1e-7);
...
const idleSmooth=(x:number)=>{x=Math.min(1,Math.max(0,x));return x*x*(3-2*x);};
...
function applyIdleNonthumb(time:number,side:'L'|'R',fi:number,bones:THREE.Bone[],wrist:THREE.Bone,base:THREE.Vector3,previous:Anchor|undefined,next:Anchor|undefined){
 const hi=side==='L'?0:1;
 if(fi===1){
  const c=idleNonthumbData.supportedIndexGaps.find(c=>matches(c,hi,fi,previous,next));
  if(c){const amount=idleSmooth((base.y-KEY_TOP-.005)/.004)*idleSmooth((base.z-.244)/.008)*idleSmooth((time-c.previousEnd)/.25)*idleSmooth((c.nextTime-time)/.32);rotate(bones[0],new THREE.Vector3(1,0,0),5*amount);}
 }
...
export class Pianist extends OriginalPianist {
 plannedPose(hand,time){const pose=super.plannedPose(hand,time);if(hand.side==='R'){for(const [end,start,dy,dz]of[[106.81,106.919774,.020,.024],[107.18,107.269683,.024,0]]){if(time<=end||time>=start)continue;const u=(time-end)/(start-end),w=Math.sin(u*Math.PI)**2;pose.position.y+=dy*w;pose.position.z+=dz*w;}}return pose;}
 update(...args){super.update(...args);releaseRepair(this,args[0],args[2]);}
}
