...
class OriginalPianist {
 ...
 this.openingPalmFrame=Math.max(this.openingPalmFrame,hand.side==='R'?smooth((time-106.64)/.18)*(1-smooth((time-107.36)/.19)):0);this.chordPalmFrame=hand.side==='R'?smooth((time-198.348852)/(198.670185-198.348852))*(1-smooth((time-199.66868)/(200.026507-199.66868))):0;
...
function phraseRepair(player,time){
 ...
 for(let fi=1;fi<5;fi++){
  ...
  if(previous){const end=previous.time+previous.duration,weight=1-smooth((time-end)/.09);if(weight>0){const a=contact(previous,end);rotations.forEach((q,j)=>q.slerp(a[j],weight));}}
  if(next){const weight=1-smooth((next.time-time)/.16);if(weight>0){const b=contact(next,next.time);rotations.forEach((q,j)=>q.slerp(b[j],weight));}}
