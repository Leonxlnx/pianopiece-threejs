...
function releaseRepair(player,time){
 ...
 for(let fi=1;fi<5;fi++){
  ...
  if(fi===1&&previous&&next&&['p00519','p00524'].includes(previous.id)){
   ...
   const w=smooth(since/.19)*smooth(before/.27);const neutral=f.rest.map(q=>q.clone());neutral[0].premultiply(new THREE.Quaternion().setFromAxisAngle(v3(-1,0,0),(player.transitionParams?.idleLift??0)*Math.PI/180));rotations.forEach((q,j)=>q.copy(f.bones[j].quaternion).slerp(neutral[j],w));
  }
