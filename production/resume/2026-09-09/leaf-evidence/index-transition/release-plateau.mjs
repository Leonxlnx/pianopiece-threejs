...
function releaseRepair(player,time){
 ...
 for(let fi=1;fi<5;fi++){
  ...
  if(fi===1&&previous&&next&&['p00519','p00524'].includes(previous.id)){
   const end=previous.time+previous.duration,gap=next.time-end,u=(time-end)/gap,a=contact(previous,end),b=contact(next,next.time);rotations=a.map((q,j)=>q.slerp(b[j],smooth(u)));rotations[0].premultiply(new THREE.Quaternion().setFromAxisAngle(v3(-1,0,0),(smooth(u/.28)*(1-smooth((u-(player.transitionParams?.hold??.65))/(1-(player.transitionParams?.hold??.65)))))*(player.transitionParams?.arc??30)*Math.PI/180));
   rotations[0].premultiply(new THREE.Quaternion().setFromAxisAngle(v3(0,0,-1),(smooth(u/.28)*(1-smooth((u-(player.transitionParams?.hold??.65))/(1-(player.transitionParams?.hold??.65)))))*(player.transitionParams?.spread??0)*Math.PI/180));
  }else{
