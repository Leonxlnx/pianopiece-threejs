...
function releaseRepair(player,time){
 ...
 for(let fi=1;fi<5;fi++){
  ...
  if(fi===1&&previous&&next&&['p00519','p00524'].includes(previous.id)){
   const end=previous.time+previous.duration,gap=next.time-end,u=(time-end)/gap,a=contact(previous,end),b=contact(next,next.time);rotations=a.map((q,j)=>q.slerp(b[j],smooth(u)));rotations[0].premultiply(new THREE.Quaternion().setFromAxisAngle(v3(-1,0,0),Math.sin(u*Math.PI)**2*(player.transitionParams?.arc??30)*Math.PI/180));
   rotations[0].premultiply(new THREE.Quaternion().setFromAxisAngle(v3(0,0,-1),Math.sin(u*Math.PI)**2*(player.transitionParams?.spread??0)*Math.PI/180));
  }else{
   const since=previous?time-previous.time-previous.duration:100,before=next?next.time-time:100;
   const w=smooth(since/.19)*smooth(before/.27);const neutral=f.rest.map(q=>q.clone()),wristQ=hand.wrist.getWorldQuaternion(new THREE.Quaternion()),axis=v3(-1,0,0).applyQuaternion(wristQ);let parentQ=wristQ.clone();for(let j=0;j<3;j++){const angle=[player.transitionParams?.idleLift??40,-(player.transitionParams?.pip??35),-(player.transitionParams?.dip??20)][j],localAxis=axis.clone().applyQuaternion(parentQ.clone().invert());neutral[j].premultiply(new THREE.Quaternion().setFromAxisAngle(localAxis,angle*Math.PI/180));parentQ.multiply(neutral[j]);}rotations.forEach((q,j)=>q.copy(f.bones[j].quaternion).slerp(neutral[j],w));
  }
...
export class Pianist extends OriginalPianist {
 plannedPose(hand,time){const pose=super.plannedPose(hand,time);if(hand.side==='R'){for(const [end,start]of[[106.81,106.923274],[107.18,107.269683]]){if(time<=end||time>=start)continue;const u=(time-end)/(start-end),w=Math.sin(u*Math.PI)**2;pose.position.y+=(this.transitionParams?.wristY??.015)*w;pose.position.z+=(this.transitionParams?.wristZ??.015)*w;}}return pose;}
