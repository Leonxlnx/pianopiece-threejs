...
export class Pianist extends OriginalPianist {
 plannedPose(hand,time){const pose=super.plannedPose(hand,time);if(hand.side==='R'){for(const [end,start]of[[106.81,106.923274],[107.18,107.269683]]){if(time<=end||time>=start)continue;const u=(time-end)/(start-end),w=Math.sin(u*Math.PI)**2;pose.position.y+=(.024)*w;pose.position.z+=(.024)*w;}}return pose;}
 update(...args){super.update(...args);releaseRepair(this,args[0]);}
}
