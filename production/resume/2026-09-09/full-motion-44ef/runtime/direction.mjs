...
export class Direction {
    ...
    constructor(score) {
        ...
        const plan = [
            ...
            [atBar(21), 'Across the strings', [1.48, 2.75, -.54], [1.75, 2.34, .30], [0, .77, -.53], 46, 44],
    ...
    update(camera, time, aspect) {
        ...
        this.name = s.name;
        camera.position.copy(s.from).lerp(s.to, this.reduce ? .35 : t);
        ...
        if (aspect < 1) {
            ...
            if (s.name === 'Across the strings') {
                ...
                // Portrait aligns the keyboard vertically and keeps the raised performer
                // outside the hand detail, while preserving both active hand regions.
                const left = /left hand/i.test(s.name);
                camera.position.set((left ? -1 : 1) * mix(.12, .09, t), mix(1.75, 1.72, t), .12);
                look.set(0, .754, .24);
                camera.up.set(1, 0, 0);
                fov = 52;
            }
