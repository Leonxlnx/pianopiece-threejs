...
import { KEY_TOP } from './piano.mjs';
import { idleNonthumbData } from './idle-nonthumb-data.mjs';
...
export function applyIdleNonthumb(time, side, fi, bones, wrist, base, previous, next) {
    const hi = side === 'L' ? 0 : 1;
    if (fi === 1) {
        const c = idleNonthumbData.supportedIndexGaps.find(c => matches(c, hi, fi, previous, next));
        if (c) {
            const amount = smooth((base.y - KEY_TOP - .005) / .004) * smooth((base.z - .244) / .008) * smooth((time - c.previousEnd) / .25) * smooth((c.nextTime - time) / .32) * (c.earlyRelease ? 1 - smooth((time - c.earlyRelease[0]) / (c.earlyRelease[1] - c.earlyRelease[0])) : 1);
