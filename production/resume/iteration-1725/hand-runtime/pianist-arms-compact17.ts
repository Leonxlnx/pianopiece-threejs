...
export class Pianist {
 ...
 if(hand.side==='R'&&time>=6.0390358363389325&&time<=12.392821){const weight=smooth((time-6.0390358363389325)/.10)*(1-smooth((time-11.85)/.54)),neutral=localPose(wp(hand.wrist),pose.q,v3(keyX(62)+.01225*smooth((time-10.95)/.18)*(1-smooth((time-11.43)/.24)),.757,.267));idlePose.forEach((q,j)=>q.slerp(neutral[j],weight));}
 ...
 const openingRest=hand.side==='R'?smooth((time-6.0390358363389325)/.10)*(1-smooth((time-11.85)/.54)):0;
 if(openingRest>0){const midi=[62,64,67,69,71][fi],touch=v3(keyX(midi),KEY_TOP+.016,.235),reference=localPose(wp(hand.wrist),pose.q,touch);idlePose.forEach((q,j)=>q.slerp(reference[j],openingRest));}
 const contactPose=(note:Note,at:number)=>{const anchor=this.plannedPose(hand,at),origin=finger.bones[0].position.clone().applyQuaternion(anchor.q).add(anchor.position),z=note.contactZ??this.contactDepth(origin.z,isBlack(note.midi),fi),touch=v3(keyX(note.midi),(mix(keySurfaceY(note.midi,z,1),piano.contact(note.midi,z).y,openingRest))+(note.contactLift??.002),z);return localPose(anchor.position,anchor.q,touch,note.thumbOpposition??0);};
 const previous=prev&&prev.time+prev.duration<=time?prev:undefined,next=nxt&&nxt.time>=time?nxt:undefined;
 const end=previous?previous.time+previous.duration:-10,start=next?.time??1e6,gap=start-end;
 let rotations=idlePose.map(q=>q.clone());
