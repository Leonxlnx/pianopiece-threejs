...
import { wristMotionSampler } from '/workspace/scratch/2e8cc8e77f98/active-hand-fit/wrist-motion-arc.mjs';
...
export class Pianist {
    ...
    fingerPoints(base, target, f, fi, palmQ, opposition = 0) {
        ...
        if (fi === 0) {
            ...
            const q = palmQ ?? f.bones[0].parent.getWorldQuaternion(new THREE.Quaternion());
            const reference = v3(-1, 0, 0).applyQuaternion(q);
            normal = reference.addScaledVector(e, -reference.dot(e));
            if (normal.lengthSq() < 1e-8) {
                ...
                normal = stable.lerp(natural, smooth((horizontal - .004) / .024));
            }
            normal.normalize();
        }
