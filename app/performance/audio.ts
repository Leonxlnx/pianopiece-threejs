...
export class PerformanceAudio {
 ...
 private renderedEnd = false;
 private meter?: Float32Array<ArrayBuffer>;
 ...
 async load(progress: (n: number) => void) {
  ...
  context.onstatechange = () => {
   ...
   if (!this.disposed && this.playing && this.source && context.state !== 'running') this.pause();
  };
  this.fetchController = new AbortController();
  const response = await fetch('/assets/daybreak.mp3', { signal: this.fetchController.signal });
  if (!response.ok) throw new Error('The recording could not load. Please try again.');
  ...
  if (reader) {
   ...
   while (true) {
    ...
    if (total && !this.disposed) progress(Math.min(.92, received / total * .92));
   }
   const combined = new Uint8Array(received);
   let i = 0;
   for (const c of chunks) { combined.set(c, i); i += c.length; }
 ...
 async play() {
  if (this.disposed || !this.context || !this.buffer || !this.gain || this.playing) return;
  const context = this.context;
  const generation = ++this.generation;
  if (this.offset >= this.duration - .001) this.offset = 0;
  ...
  try {
   ...
   src.onended = () => {
    ...
    } else { src.disconnect(); envelope.disconnect(); }
   };
 ...
 private releaseSource() {
  ...
  try {
   if (context && context.state === 'running' && envelope) {
    ...
    src.stop(now + .018);
   } else { src.stop(); src.disconnect(); envelope?.disconnect(); }
  } catch { src.disconnect(); envelope?.disconnect(); }
 }

 pause() {
  ++this.generation; // Cancels a resume even when no source has been created yet.
  if (this.playing) this.offset = this.time();
  this.playing = false;
  this.renderedEnd = false;
  this.releaseSource();
 }
 ...
 async seek(t: number) {
  ...
  if (wasPlaying && !this.ended) await this.play();
 }
 ...
 setVolume(v: number) {
  this.volume = clamp(v);
  if (this.gain && this.context) this.gain.gain.setTargetAtTime(this.volume, this.context.currentTime, .025);
 }

 rms() {
  if (!this.analyser || !this.meter) return 0;
