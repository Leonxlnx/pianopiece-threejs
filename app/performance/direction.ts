...
export type ShotKind = 'wide'|'performance'|'hands'|'left-hand'|'portrait'|'interior';
export interface Shot { kind:ShotKind; start:number;end:number;name:string;from:THREE.Vector3;to:THREE.Vector3;look:THREE.Vector3;lookTo:THREE.Vector3;fov:number;fovTo:number; }
...
export class Direction {
 ...
 const kinds:ShotKind[]=['wide','performance','hands','performance','wide','interior','wide','wide','hands','portrait','performance','left-hand','performance','wide','wide','hands','portrait','left-hand','wide','hands','performance','hands','performance','wide'];
 ...
 this.name=s.name;camera.position.copy(s.from).lerp(s.to,t);const look=s.look.clone().lerp(s.lookTo,t);
 ...
 const left=s.kind==='left-hand';camera.position.set(look.x+(left?-1:1)*1.40,1.25,.25);
 look.set(look.x,.754,.25);fov=26;
 }else{
